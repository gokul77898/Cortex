...
function parseLaunchOptions(argv: string[]): LaunchOptions {
  ...
  let fast = false
  let goal = normalizeRecommendationGoal(process.env.CORTEX_PROFILE_GOAL)
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if ((lower === 'auto' || lower === 'openai' || lower === 'ollama' || lower === 'codex' || lower === 'gemini' || lower === 'atomic-chat' || lower === 'huggingface') && requestedProfile === 'auto') {
...
function applyFastFlags(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  env.CORTEX_SIMPLE ??= '1'
  env.CORTEX_DISABLE_THINKING ??= '1'
  env.DISABLE_INTERLEAVED_THINKING ??= '1'
  env.DISABLE_AUTO_COMPACT ??= '1'
  env.CORTEX_DISABLE_AUTO_MEMORY ??= '1'
  env.CORTEX_DISABLE_BACKGROUND_TASKS ??= '1'
  return env
}
...
function printSummary(profile: ProviderProfile): void {
  ...
  } else if (profile === 'huggingface') {
    console.log('Using configured Hugging Face provider settings.')
  } else {
...
async function main(): Promise<void> {
  ...
  if (!requestedProfile) {
    console.error('Usage: bun run scripts/provider-launch.ts [openai|ollama|codex|gemini|atomic-chat|huggingface|auto] [--fast] [--goal <latency|balanced|coding>] [-- <cli args>]')
    process.exit(1)
  }
