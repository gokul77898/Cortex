...
function currentBaseUrl(): string {
  if (isTruthy(process.env.CORTEX_USE_GEMINI)) {
    return process.env.GEMINI_BASE_URL ?? GEMINI_DEFAULT_BASE_URL
  }
  if (isTruthy(process.env.CORTEX_USE_GITHUB)) {
...
function checkOpenAIEnv(): CheckResult[] {
  const results: CheckResult[] = []
  const useGemini = isTruthy(process.env.CORTEX_USE_GEMINI)
  const useGithub = isTruthy(process.env.CORTEX_USE_GITHUB)
  const useOpenAI = isTruthy(process.env.CORTEX_USE_OPENAI)
  ...
  if (!useOpenAI) {
    results.push(pass('Provider mode', 'CORTEX login flow enabled (CORTEX_USE_OPENAI is off).'))
    return results
  }
...
async function checkBaseUrlReachability(): Promise<CheckResult> {
  ...
  if (request.baseUrl.includes('huggingface.co')) {
    return pass(
      'Provider reachability',
      'Skipped for Hugging Face Router (reliability/timeout variance in /models probe).',
    )
  }
...
function checkOllamaProcessorMode(): CheckResult {
  if (
    !isTruthy(process.env.CORTEX_USE_OPENAI) ||
    isTruthy(process.env.CORTEX_USE_GEMINI) ||
    isTruthy(process.env.CORTEX_USE_GITHUB)
  ) {
...
function serializeSafeEnvSummary(): Record<string, string | boolean> {
  ...
  return {
    CORTEX_USE_OPENAI: isTruthy(process.env.CORTEX_USE_OPENAI),
