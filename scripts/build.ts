/**
 * CORTEX build script — bundles the TypeScript source into a single
...
// Most CORTEX-internal features stay off; open-build features can be
...
const result = await Bun.build({
  ...
  define: {
    ...
    // version separately in CORTEX branding.
    'MACRO.VERSION': JSON.stringify('99.0.0'),
    'MACRO.DISPLAY_VERSION': JSON.stringify(version),
    'MACRO.BUILD_TIME': JSON.stringify(new Date().toISOString()),
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/anthropics/cortex-code/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('@gitlawb/cortex'),
...
export const createCORTEXForChromeMcpServer = noop;
...
console.log(`✓ Built cortex v${version} → dist/cli.mjs`)
