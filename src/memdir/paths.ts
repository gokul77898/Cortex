 ...
 *   1. CORTEX_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. CORTEX_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no CORTEX_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.CORTEX_DISABLE_AUTO_MEMORY
 ...
 * Resolution order:
 *   1. CORTEX_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.cortex (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.CORTEX_REMOTE_MEMORY_DIR) {
    return process.env.CORTEX_REMOTE_MEMORY_DIR
  }
 ...
 *
 * SECURITY: projectSettings (.cortex/settings.json committed to the repo) is
 ...
 * same repo share one auto-memory directory (anthropics/cortex-code#24382).
 */
