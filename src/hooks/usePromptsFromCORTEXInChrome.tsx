...
import { CLAUDE_IN_CHROME_MCP_SERVER_NAME, isTrackedCORTEXInChromeTabId } from '../utils/cortexInChrome/common.js';
import { lazySchema } from '../utils/lazySchema.js';
import { enqueuePendingNotification } from '../utils/messageQueueManager.js';

// Schema for the prompt notification from Chrome extension (JSON-RPC 2.0 format)
const CORTEXInChromePromptNotificationSchema = lazySchema(() => z.object({
...
/**
 * A hook that listens for prompt notifications from the CORTEX for Chrome extension,
 * enqueues them as user prompts, and syncs permission mode changes to the extension.
 */
export function usePromptsFromCORTEXInChrome(mcpClients, toolPermissionMode) {
  const $ = _c(6);
  useRef(undefined);
  let t0;
  if ($[0] !== mcpClients) {
