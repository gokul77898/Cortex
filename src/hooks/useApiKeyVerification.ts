...
import { verifyApiKey } from '../services/api/cortex.js'
import {
...
export function useApiKeyVerification(): ApiKeyVerificationResult {
  const [status, setStatus] = useState<VerificationStatus>(() => {
    if (!isCORTEXAuthEnabled() || isCORTEXAISubscriber()) {
      return 'valid'
    }
  ...
  const verify = useCallback(async (): Promise<void> => {
    ...
    // all sources. getCORTEXApiKeyWithSource() reads the now-warm cache.
    await getApiKeyFromApiKeyHelper(getIsNonInteractiveSession())
    const { key: apiKey, source } = getCORTEXApiKeyWithSource()
    if (!apiKey) {
