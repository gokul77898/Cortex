...
async function _temp2() {
  ...
  const subscriptionType = await getExistingCORTEXSubscription();
  ...
  return {
    ...
    jsx: <Text color="suggestion">Use your existing CORTEX {subscriptionType} plan with CORTEX<Text color="text" dimColor={true}>{" "}· /login to activate</Text></Text>,
    priority: "low"
  };
}
...
async function getExistingCORTEXSubscription(): Promise<'Max' | 'Pro' | null> {
  ...
  if (profile.account.has_cortex_max) {
    return 'Max';
  }
  if (profile.account.has_cortex_pro) {
    return 'Pro';
  }
  return null;
}
