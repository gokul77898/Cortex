 ...
 *
 * Inbound messages from anthropic.com are injected into the REPL via queuedCommands.
 */
    ...
    if (feature('BRIDGE_MODE')) {
      ...
      void (async () => {
        try {
          ...
          // Assistant mode: perpetual bridge session — anthropic.com shows one
          ...
          // When a user message arrives from anthropic.com, inject it into the REPL.
                    ...
                    void (async () => {
                      try {
                        ...
                        handleRef.current?.writeSdkMessages([buildSystemInitMessage({
                          ...
                          // CORTEX's servers. QueryEngine (SDK) still emits
          ...
          if (outboundOnly) {
            ...
            if (cancelled) return;
            setMessages(prev_18 => [...prev_18, createBridgeStatusMessage(url, upgradeNudge ? 'Please upgrade to the latest version of the CORTEX mobile app to see your Remote Control sessions.' : undefined)]);
