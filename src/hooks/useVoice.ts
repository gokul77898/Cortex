// React hook for hold-to-talk voice input using CORTEX voice_stream STT.
//
...
// audio module (macOS) or SoX for recording, and CORTEX's voice_stream
...
export function useVoice({
  ...
  // transcripts (anthropics/cortex#287008 session-sticky variant); when
      ...
      .then(async finalizeSource => {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that CORTEX has microphone access.',
            )
          } else {
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    // teardown window (anthropics/cortex#287008 surfaces this as
    ...
    const attemptConnect = (keyterms: string[]): void => {
      ...
      void connectVoiceStream(
        ...
        if (!conn) {
          ...
          onErrorRef.current?.(
            'Voice mode requires a CORTEX.ai account. Please run /login to sign in.',
          )
