...
/**
 * Surfaces plugin-install prompts driven by `<cortex-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/cortex-code-hints.md.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/cortexCodeHints.js';
...
type UseCORTEXCodeHintRecommendationResult = {
  recommendation: PluginHintRecommendation | null;
  handleResponse: (response: 'yes' | 'no' | 'disable') => void;
};
export function useCORTEXCodeHintRecommendation() {
  const $ = _c(11);
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useCORTEXCodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
