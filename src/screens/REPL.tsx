...
import type { ContentBlockParam, ImageBlockParam } from 'src/utils/anthropicMock.js'
...
import { usePromptsFromCORTEXInChrome } from 'src/hooks/usePromptsFromCORTEXInChrome.js';
...
import { useCORTEXCodeHintRecommendation } from 'src/hooks/useCORTEXCodeHintRecommendation.js';
import { PluginHintMenu } from 'src/components/CORTEXCodeHint/PluginHintMenu.js';
...
// repin to bottom. Josh Rosen's workflow: CORTEX emits long output → scroll
// up to read the start → start typing → before this fix, snapped to bottom.
// https://cortex.slack.com/archives/C07VBSHV7EV/p1773545449871739
...
export type Props = {
  ...
  // Direct connect config for `cortex connect` mode (connects to a cortex server)
  directConnectConfig?: DirectConnectConfig;
  // SSH session for `cortex ssh` mode (local REPL, remote tools over ssh)
...
export function REPL({
  ...
  const titleDisabled = useMemo(() => isEnvTruthy(process.env.CORTEX_DISABLE_TERMINAL_TITLE), []);
  const moreRightEnabled = useMemo(() => "external" === 'ant' && isEnvTruthy(process.env.CLAUDE_MORERIGHT), []);
  const disableVirtualScroll = useMemo(() => isEnvTruthy(process.env.CORTEX_DISABLE_VIRTUAL_SCROLL), []);
  const disableMessageActions = feature('MESSAGE_ACTIONS') ?
    // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
    useMemo(() => isEnvTruthy(process.env.CORTEX_DISABLE_MESSAGE_ACTIONS), []) : false;
  ...
  // from CORTEX_NO_FLICKER=0 (which is process-lifetime) — this is
  ...
  } = useCORTEXCodeHintRecommendation();
  ...
  // Allow CORTEX in Chrome MCP to send prompts through MCP notifications
  // and sync permission mode changes to the Chrome extension
  usePromptsFromCORTEXInChrome(isRemoteSession ? EMPTY_MCP_CLIENTS : mcpClients, toolPermissionContext.mode);
  ...
  // This enables "immediate" commands (like /btw) to persist while CORTEX is processing.
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'CORTEX';
  ...
  // Prevent macOS from sleeping while CORTEX is working
  useEffect(() => {
  ...
  // Push status to the PID file for `cortex ps`. Fire-and-forget; ps falls
  ...
  // Direct connect hook - manages WebSocket to a cortex server for `cortex connect` mode
  ...
  // SSH session hook - manages ssh child process for `cortex ssh` mode.
  ...
  useEffect(() => {
    ...
    setMessages(prev => [...prev, createSystemMessage(`Worktree creation took ${secs}s. For large repos, set \`worktree.sparsePaths\` in .cortex/settings.json to check out only the directories you need — e.g. \`{"worktree": {"sparsePaths": ["src", "packages/foo"]}}\`.`, 'info')]);
  }, [setMessages]);
  ...
  const resume = useCallback(async (sessionId: UUID, log: LogOption, entrypoint: ResumeEntrypoint) => {
    ...
    try {
      ...
      // JSONL with the fork's sessionId, so `cortex -r {forkId}` also works.
  ...
  // This allows CORTEX to edit files that were read in previous sessions
  ...
  const sandboxAskCallback: SandboxAskCallback = useCallback(async (hostPattern: NetworkHostPattern) => {
    ...
    return new Promise(resolveShouldAllowHost => {
      ...
      // remote user (e.g. on anthropic.com) can approve it too.
  ...
  const getToolUseContext = useCallback((messages: MessageType[], newMessages: MessageType[], abortController: AbortController, mainLoopModel: string): ProcessUserInputContext => {
    ...
    return {
      ...
      onCompactProgress: event => {
        switch (event.type) {
          case 'hooks_start':
            setSpinnerColor('cortexBlue_FOR_SYSTEM_SPINNER');
            setSpinnerShimmerColor('cortexBlueShimmer_FOR_SYSTEM_SPINNER');
  ...
  const onQueryImpl = useCallback(async (messagesIncludingNewMessages: MessageType[], newMessages: MessageType[], abortController: AbortController, shouldQuery: boolean, additionalAllowedTools: string[], mainLoopModelParam: string, effort?: EffortValue) => {
    ...
    // Mark onboarding as complete when any user message is sent to CORTEX
    ...
    // title silently fell through to the "CORTEX" default.
  ...
  const onSubmit = useCallback(async (input: string, helpers: PromptInputHelpers, speculationAccept?: {
    ...
    // even while CORTEX is processing. Commands opt-in via `immediate: true`.
    ...
    // controls treatment: "dialog" (blocking), "hint" (notification), "off".
    {
      ...
      const idleThresholdMin = Number(process.env.CORTEX_IDLE_THRESHOLD_MINUTES ?? 75);
      const tokenThreshold = Number(process.env.CORTEX_IDLE_TOKEN_THRESHOLD ?? 100_000);
  ...
  // Handlers for auto-run /issue or /good-cortex (defined after onSubmit)
  ...
  // for remote access via anthropic.com. No-op in external builds or when not enabled.
  const {
    sendBridgeResult
  ...
  // That write storm is the primary trigger for ~/.cortex.json corruption
  // (GH #3117).
  ...
  // Show notification when CORTEX is done responding and user is idle
  useEffect(() => {
    // Don't set up notification if CORTEX is busy
    if (isLoading) return;
    ...
    const timer = setTimeout((lastQueryCompletionTime, isLoading, toolJSX, focusedInputDialogRef, terminal) => {
      ...
      if (lastUserInteraction > lastQueryCompletionTime) {
        // User has interacted since CORTEX finished - they're not idle, don't notify
        return;
      }
      ...
      if (!isLoading && !toolJSX &&
        ...
        void sendNotification({
          message: 'CORTEX is waiting for your input',
  ...
  useEffect(() => {
    ...
    const idleThresholdMs = Number(process.env.CORTEX_IDLE_THRESHOLD_MINUTES ?? 75) * 60_000;
  ...
  // Scheduled tasks from .cortex/scheduled_tasks.json (CronCreate/Delete/List)
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nCORTEX has been suspended. Run \`fg\` to bring CORTEX back.\nNote: ctrl + z now suspends CORTEX, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
    ...
    <MCPConnectionManager key={remountKey} dynamicMcpConfig={dynamicMcpConfig} isStrictMcpConfig={strictMcpConfig}>
      <FullscreenLayout scrollRef={scrollRef} overlay={toolPermissionOverlay} bottomFloat={isBuddyEnabled() && companionVisible && !companionNarrow ? <CompanionFloatingBubble /> : undefined} modal={centeredModal} modalScrollRef={modalScrollRef} dividerYRef={dividerYRef} hidePill={!!viewedAgentTask} hideSticky={!!viewedTeammateTask} newMessageCount={unseenDivider?.count ?? 0} onPillClick={() => {
        ...
        {/* Hide the processing placeholder while a modal is showing —
                  ...
                  the user sees their input echoed while CORTEX processes. */}
        ...
        <Box flexDirection="column" flexGrow={1}>
          ...
          {!toolJSX?.shouldHidePromptInput && !focusedInputDialog && !isExiting && !disabled && !cursor && !isShuttingDown() && <>
            ...
            {postCompactSurvey.state !== 'closed' ? <FeedbackSurvey state={postCompactSurvey.state} lastResponse={postCompactSurvey.lastResponse} handleSelect={postCompactSurvey.handleSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={handleSurveyRequestFeedback} /> : memorySurvey.state !== 'closed' ? <FeedbackSurvey state={memorySurvey.state} lastResponse={memorySurvey.lastResponse} handleSelect={memorySurvey.handleSelect} handleTranscriptSelect={memorySurvey.handleTranscriptSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={handleSurveyRequestFeedback} message="How well did CORTEX use its memory? (optional)" /> : <FeedbackSurvey state={feedbackSurvey.state} lastResponse={feedbackSurvey.lastResponse} handleSelect={feedbackSurvey.handleSelect} handleTranscriptSelect={feedbackSurvey.handleTranscriptSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={didAutoRunIssueRef.current ? undefined : handleSurveyRequestFeedback} />}
