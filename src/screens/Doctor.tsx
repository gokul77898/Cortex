...
import { getCORTEXConfigHomeDir } from 'src/utils/envUtils.js';
...
export function Doctor(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    const envVars = [{
      ...
      name: "CORTEX_MAX_OUTPUT_TOKENS",
      ...getModelMaxOutputTokens("cortex-opus-4-6")
    }];
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        const userAgentsDir = join(getCORTEXConfigHomeDir(), "agents");
        const projectAgentsDir = join(getOriginalCwd(), ".cortex", "agents");
        const {
          activeAgents,
          allAgents,
          failedFiles
        ...
        if (isPidBasedLockingEnabled()) {
          const locksDir = join(getXDGStateHome(), "anthropic", "locks");
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("CORTEX diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
  ...
  if ($[73] !== contextWarnings) {
    t39 = contextWarnings && (contextWarnings.cortexMdWarning || contextWarnings.agentWarning || contextWarnings.mcpWarning) && <Box flexDirection="column"><Text bold={true}>Context Usage Warnings</Text>{contextWarnings.cortexMdWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.cortexMdWarning.message}</Text></Text><Text>{"  "}└ Files:</Text>{contextWarnings.cortexMdWarning.details.map(_temp16)}</>}{contextWarnings.agentWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.agentWarning.message}</Text></Text><Text>{"  "}└ Top contributors:</Text>{contextWarnings.agentWarning.details.map(_temp17)}</>}{contextWarnings.mcpWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.mcpWarning.message}</Text></Text><Text>{"  "}└ MCP servers:</Text>{contextWarnings.mcpWarning.details.map(_temp18)}</>}</Box>;
    $[73] = contextWarnings;
    $[74] = t39;
  } else {
    t39 = $[74];
