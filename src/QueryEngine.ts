...
import { accumulateUsage, updateUsage } from 'src/services/api/cortex.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // a memory directory and needs CORTEX to know how to use it (which
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.CORTEX_EAGER_FLUSH) ||
          isEnvTruthy(process.env.CORTEX_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via CORTEX_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or CORTEX_PLUGIN_SEED_DIR before this runs;
    ...
    for await (const message of query({
      ...
      if (
        ...
        // If the SDK subprocess restarts before then (cortex-desktop kills
        ...
        if (persistSession) {
          // Fire-and-forget for assistant messages. cortex.ts yields one
      ...
      switch (message.type) {
        ...
        case 'stream_event':
          ...
          if (message.event.type === 'message_delta') {
            ...
            // real value only arrives here (see cortex.ts message_delta
...
/**
 * Sends a single prompt to the CORTEX API and returns the response.
 * Assumes that cortex is being used non-interactively -- will not
