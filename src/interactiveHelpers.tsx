...
import { getExternalCORTEXMdIncludes, getMemoryFiles, shouldShowCORTEXMdExternalIncludesWarning } from './utils/cortexmd.js';
...
import { usesCORTEXAccountFlow } from './utils/model/providers.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], cortexInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  if ("production" === 'test' || isEnvTruthy(false) || process.env.IS_DEMO // Skip onboarding in demo mode
  ) {
    return false;
  }

  const usesCORTEXSetup = usesCORTEXAccountFlow();
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    // Skip trust dialog UI for third-party providers (no CORTEX auth), but still
    // run trust state initialization below so the REPL mounts correctly.
    if (usesCORTEXSetup && !checkHasTrustDialogAccepted()) {
      const {
        TrustDialog
    ...
    if (usesCORTEXSetup) {
      ...
      // Check for cortex.md includes that need approval
      if (await shouldShowCORTEXMdExternalIncludesWarning()) {
        const externalIncludes = getExternalCORTEXMdIncludes(await getMemoryFiles(true));
        const {
          CORTEXMdExternalIncludesDialog
        } = await import('./components/CORTEXMdExternalIncludesDialog.js');
        await showSetupDialog(root, done => <CORTEXMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
      }
    }
  }
  ...
  // processes but ignored by CORTEX itself (see auth.ts).
  ...
  if (feature('KAIROS') || feature('KAIROS_CHANNELS')) {
    ...
    if (devChannels && devChannels.length > 0) {
      ...
      if (!isChannelsEnabled() || !getCORTEXAIOAuthTokens()?.accessToken) {
  ...
  // Show Chrome onboarding for first-time CORTEX in Chrome users
  if (cortexInChrome && !getGlobalConfig().hasCompletedCORTEXInChromeOnboarding) {
    const {
      CORTEXInChromeOnboarding
    } = await import('./components/CORTEXInChromeOnboarding.js');
    await showSetupDialog(root, done => <CORTEXInChromeOnboarding onDone={done} />);
  }
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.CORTEX_FRAME_TIMING_LOG;
  return {
