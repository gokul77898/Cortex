...
export function getLocalISODate(): string {
  ...
  if (process.env.CORTEX_OVERRIDE_DATE) {
    return process.env.CORTEX_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.CORTEX_OVERRIDE_DATE
    ? new Date(process.env.CORTEX_OVERRIDE_DATE)
    : new Date()
