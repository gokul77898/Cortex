 ...
 * CORTEX_DISABLE_NONESSENTIAL_TRAFFIC are set; a `false` default would
 ...
 *
 * `CORTEX_DISABLE_CRON` is a local override that wins over GB.
 */
export function isKairosCronEnabled(): boolean {
  return feature('AGENT_TRIGGERS')
    ? !isEnvTruthy(process.env.CORTEX_DISABLE_CRON) &&
 ...
 * durable cron. Does NOT consult CORTEX_DISABLE_CRON (that kills the whole
...
export function buildCronCreateDescription(durableEnabled: boolean): string {
  return durableEnabled
    ? 'Schedule a prompt to run at a future time — either recurring on a cron schedule, or once at a specific time. Pass durable: true to persist to .cortex/scheduled_tasks.json; otherwise session-only.'
    : 'Schedule a prompt to run at a future time within this CORTEX session — either recurring on a cron schedule, or once at a specific time.'
}
...
Jobs live only in this CORTEX session — nothing is written to disk, and the job is gone when CORTEX exits.`
  ...
  const durableRuntimeNote = durableEnabled
    ? 'Durable jobs persist to .cortex/scheduled_tasks.json and survive session restarts — on next launch they resume automatically. One-shot durable tasks that were missed while the REPL was closed are surfaced for catch-up. Session-only jobs die with the process. '
    : ''
...
export function buildCronDeletePrompt(durableEnabled: boolean): string {
  return durableEnabled
    ? `Cancel a cron job previously scheduled with ${CRON_CREATE_TOOL_NAME}. Removes it from .cortex/scheduled_tasks.json (durable jobs) or the in-memory session store (session-only jobs).`
...
export function buildCronListPrompt(durableEnabled: boolean): string {
  return durableEnabled
    ? `List all cron jobs scheduled via ${CRON_CREATE_TOOL_NAME}, both durable (.cortex/scheduled_tasks.json) and session-only.`
