...
function isPathAllowed(
  ...
  // have explicitly configured writable directories (e.g. /tmp/cortex/) —
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.cortex; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.cortex/settings.json (CORTEX's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.cortex`, validate subsequent statements with
  // cwd='./.cortex'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, CORTEX may only access files in the allowed working directories for this session: ${dirListStr}.`
