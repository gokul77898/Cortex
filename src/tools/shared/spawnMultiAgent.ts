...
/**
 * Ensures a team file exists on disk. If it doesn't (e.g. when a non-CORTEX
...
async function handleSpawnSplitPane(
  ...
  // - Outside both: creates cortex-swarm session with tiled teammates
  ...
  // Build the command to spawn CORTEX with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces CORTEX_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, CORTEX_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new CORTEX instance.
