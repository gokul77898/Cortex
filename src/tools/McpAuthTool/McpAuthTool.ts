...
export function createMcpAuthTool(
  ...
  return {
    ...
    async call(_input, context) {
      // anthropic.com connectors use a separate auth flow (handleCORTEXAIAuth in
      ...
      if (config.type === 'cortexai-proxy') {
        return {
          data: {
            ...
            message: `This is a anthropic.com MCP connector. Ask the user to run /mcp and select "${serverName}" to authenticate.`,
          },
        }
      }
