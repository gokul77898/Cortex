...
export function buildCurrentProviderSummary(options?: {
  ...
  if (isEnvTruthy(processEnv.CORTEX_USE_GEMINI)) {
    return {
  ...
  if (isEnvTruthy(processEnv.CORTEX_USE_OPENAI)) {
...
export function buildProfileSaveMessage(
  ...
  lines.push('Restart CORTEX to use it.')
...
function buildUsageText(): string {
  ...
  return [
    ...
    '',
    'Choose Auto, Ollama, OpenAI-compatible, Gemini, or Codex, then save a profile for the next CORTEX restart.',
  ].join('\n')
}
...
function ProviderChooser({
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .cortex-profile.json and return to normal startup',
    })
  }
  ...
  return (
    ...
    >
      <Box flexDirection="column" gap={1}>
        <Text>
          Save a provider profile for the next CORTEX restart without
