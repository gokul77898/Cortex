...
export function ManageMarketplaces({
  ...
  useEffect(() => {
    async function loadMarketplaces() {
      try {
        ...
        // Sort: cortex-plugin-directory first, then alphabetically
        states.sort((a, b) => {
  ...
  if (internalView === 'details' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        ...
        {!isUpdating && !shouldSkipPluginAutoupdate() && selectedMarketplace.autoUpdate && <Box marginTop={1}>
              <Text dimColor>
                Auto-update enabled. CORTEX will automatically update this
  ...
  return <Box flexDirection="column">
              ...
              <Box flexDirection="column" flexGrow={1}>
                <Box flexDirection="row" gap={1}>
                  <Text bold strikethrough={state.pendingRemove} dimColor={state.pendingRemove}>
                    {state.name === 'cortex-plugins-official' && <Text color="anthropic">✻ </Text>}
                    {state.name}
                    {state.name === 'cortex-plugins-official' && <Text color="anthropic"> ✻</Text>}
                  </Text>
      ...
      {isProcessing && <Box marginTop={1}>
          <Text color="anthropic">Processing changes…</Text>
        </Box>}
