...
import { getCORTEXAIOAuthTokens, isCORTEXAISubscriber } from '../../utils/auth.js';
  ...
  try {
    ...
    if (isCORTEXAISubscriber()) {
      ...
      if (tokens?.subscriptionType && tokens?.rateLimitTier) {
        isMax20x = tokens.subscriptionType === 'max' && tokens.rateLimitTier === 'default_cortex_max_20x';
      } else if (tokens?.accessToken) {
        const profile = await getOauthProfileFromOauthToken(tokens.accessToken);
        isMax20x = profile?.organization?.organization_type === 'cortex_max' && profile?.organization?.rate_limit_tier === 'default_cortex_max_20x';
      }
      if (isMax20x) {
    ...
    const url = 'https://anthropic.com/upgrade/max';
    ...
    setTimeout(onDone, 0, 'Failed to open browser. Please visit https://anthropic.com/upgrade/max to upgrade.');
  }
  return null;
}
