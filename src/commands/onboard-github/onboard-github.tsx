...
function mergeUserSettingsEnv(model: string): { ok: boolean; detail?: string } {
  const { error } = updateSettingsForSource('userSettings', {
    env: {
      CORTEX_USE_GITHUB: '1',
      OPENAI_MODEL: model,
      CORTEX_USE_OPENAI: undefined as any,
      CORTEX_USE_GEMINI: undefined as any,
      CORTEX_USE_BEDROCK: undefined as any,
      CORTEX_USE_VERTEX: undefined as any,
      CORTEX_USE_FOUNDRY: undefined as any,
    },
  })
  if (error) {
      ...
      if (!merged.ok) {
        setErrorMsg(
          `Token saved, but settings were not updated: ${merged.detail ?? 'unknown error'}. ` +
            `Add env CORTEX_USE_GITHUB=1 and OPENAI_MODEL to ~/.cortex/settings.json manually.`,
        )
        setStep('error')
        return
      }
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text dimColor>
        ...
        and enables CORTEX_USE_GITHUB in your user settings — no export
