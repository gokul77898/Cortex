...
import { clearMemoryFileCaches, getMemoryFiles } from '../../utils/cortexmd.js';
...
function MemoryCommand({
  ...
  const handleSelectMemoryFile = async (memoryPath: string) => {
    try {
      // Create cortex directory if it doesn't exist (idempotent with recursive)
      if (memoryPath.includes(getCORTEXConfigHomeDir())) {
        await mkdir(getCORTEXConfigHomeDir(), {
          recursive: true
        });
      }
  ...
  return <Dialog title="Memory" onCancel={handleCancel} color="remember">
      <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          <Text dimColor>
            Learn more: <Link url="https://code.anthropic.com/docs/en/memory" />
          </Text>
        </Box>
      </Box>
    </Dialog>;
