...
import { getAdditionalDirectoriesForCORTEXMd, setAdditionalDirectoriesForCORTEXMd } from '../../bootstrap/state.js';
...
export async function call(onDone: LocalJSXCommandOnDone, context: LocalJSXCommandContext, args?: string): Promise<React.ReactNode> {
  ...
  const handleAddDirectory = async (path: string, remember = false) => {
    ...
    const currentDirs = getAdditionalDirectoriesForCORTEXMd();
    if (!currentDirs.includes(path)) {
      setAdditionalDirectoriesForCORTEXMd([...currentDirs, path]);
    }
