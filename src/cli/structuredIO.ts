...
export class StructuredIO {
   ...
   * Used by the bridge to feed permission responses from anthropic.com into the
   * SDK permission flow.
   *
   ...
   * stale permission prompt on anthropic.com when the SDK consumer wins the race.
   */
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'update_environment_variables') {
        ...
        // (CORTEX_SESSION_ACCESS_TOKEN) which must be readable
      ...
      if (message.type === 'control_response') {
        ...
        // request, so it can cancel the stale permission prompt on anthropic.com.
        if (
