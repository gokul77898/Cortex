 ...
 * These are dynamically imported only when `cortex plugin *` or `cortex plugin marketplace *` runs.
 */
...
export async function pluginValidateHandler(
  ...
  try {
    ...
    // If this is a plugin manifest located inside a .cortex-plugin directory,
    ...
    if (result.fileType === 'plugin') {
      ...
      if (basename(manifestDir) === '.cortex-plugin') {
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `cortex plugin install` to install a plugin.',
      )
    }
  }
