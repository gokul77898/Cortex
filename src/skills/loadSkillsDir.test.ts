...
function writeSkill(rootDir: string, skillPath: string): void {
  const skillDir = join(rootDir, '.cortex', 'skills', ...skillPath.split('/'))
...
test('loads flat and nested skills with colon namespaces', async () => {
  const configDir = mkdtempSync(join(tmpdir(), 'cortex-skills-'))
  ...
  try {
    ...
    assert.equal(nestedSkill.skillRoot, join(configDir, '.cortex', 'skills', 'git', 'commit'))
    ...
    assert.equal(
      deepSkill.skillRoot,
      join(configDir, '.cortex', 'skills', 'frontend', 'react', 'form'),
    )
  } finally {
