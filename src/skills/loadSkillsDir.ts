...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.cortex', dir)
    case 'userSettings':
      return join(getCORTEXConfigHomeDir(), dir)
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const userSkillsDir = join(getCORTEXConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.cortex', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.CORTEX_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.cortex', 'skills')
