...
// cortexApiContent.js bundles 247KB of .md strings. Lazy-load inside
// getPromptForCommand so they only enter memory when /cortex-api is invoked.
type SkillContent = typeof import('./cortexApiContent.js')
...
→ Refer to \`{lang}/cortex-api/README.md\`

**Chat UI or real-time response display:**
→ Refer to \`{lang}/cortex-api/README.md\` + \`{lang}/cortex-api/streaming.md\`

**Long-running conversations (may exceed context window):**
→ Refer to \`{lang}/cortex-api/README.md\` — see Compaction section

**Prompt caching / optimize caching / "why is my cache hit rate low":**
→ Refer to \`shared/prompt-caching.md\` + \`{lang}/cortex-api/README.md\` (Prompt Caching section)

**Function calling / tool use / agents:**
→ Refer to \`{lang}/cortex-api/README.md\` + \`shared/tool-use-concepts.md\` + \`{lang}/cortex-api/tool-use.md\`

**Batch processing (non-latency-sensitive):**
→ Refer to \`{lang}/cortex-api/README.md\` + \`{lang}/cortex-api/batches.md\`

**File uploads across multiple requests:**
→ Refer to \`{lang}/cortex-api/README.md\` + \`{lang}/cortex-api/files-api.md\`
...
export function registerCORTEXApiSkill(): void {
  registerBundledSkill({
    name: 'cortex-api',
    description:
      'Build apps with the CORTEX API or CORTEX SDK.\n' +
      'TRIGGER when: code imports `cortex`/`src/utils/anthropicMock.js`/`cortex_agent_sdk`, or user asks to use CORTEX API, CORTEX SDKs, or Agent SDK.\n' +
      'DO NOT TRIGGER when: code imports `openai`/other AI SDK, general programming, or ML/data-science tasks.',
    allowedTools: ['Read', 'Grep', 'Glob', 'WebFetch'],
    userInvocable: true,
    async getPromptForCommand(args) {
      const content = await import('./cortexApiContent.js')
