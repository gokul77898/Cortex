...
const originalSimple = process.env.CORTEX_SIMPLE
...
async function writeJsonl(entry: unknown): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-conversation-recovery-hooks-'))
  tempDirs.push(dir)
