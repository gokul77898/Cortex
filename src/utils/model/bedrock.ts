...
> {
  ...
  try {
    ...
    // Filter for CORTEX models (SYSTEM_DEFINED filtering handled in query)
    return allProfiles
...
async function createBedrockClient() {
  ...
  // Match the CORTEX Bedrock SDK's region behavior exactly:
  ...
  const skipAuth = isEnvTruthy(process.env.CORTEX_SKIP_BEDROCK_AUTH)
...
/**
 * Check if a model ID is a foundation model (e.g., "cortex.cortex-sonnet-4-5-20250929-v1:0")
 */
export function isFoundationModel(modelId: string): boolean {
  return modelId.startsWith('cortex.')
}
 ...
 * For example:
 * - "eu.cortex.cortex-sonnet-4-5-20250929-v1:0" → "eu"
 * - "us.cortex.cortex-3-7-sonnet-20250219-v1:0" → "us"
 * - "arn:aws:bedrock:ap-northeast-2:123:inference-profile/global.cortex.cortex-opus-4-6-v1" → "global"
 * - "cortex.cortex-3-5-sonnet-20241022-v2:0" → undefined (foundation model)
 * - "cortex-sonnet-4-5-20250929" → undefined (first-party format)
 */
export function getBedrockRegionPrefix(
  ...
  for (const prefix of BEDROCK_REGION_PREFIXES) {
    if (effectiveModelId.startsWith(`${prefix}.cortex.`)) {
      return prefix
    }
  }
  return undefined
 ...
 * If the model is a foundation model (cortex.*), the prefix will be added.
 * If the model is not a Bedrock model, it will be returned as-is.
 *
 * For example:
 * - applyBedrockRegionPrefix("us.cortex.cortex-sonnet-4-5-v1:0", "eu") → "eu.cortex.cortex-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("cortex.cortex-sonnet-4-5-v1:0", "eu") → "eu.cortex.cortex-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("cortex-sonnet-4-5-20250929", "eu") → "cortex-sonnet-4-5-20250929" (not a Bedrock model)
 */
export function applyBedrockRegionPrefix(
  ...
  // Check if it's a foundation model (cortex.*) and add the prefix
