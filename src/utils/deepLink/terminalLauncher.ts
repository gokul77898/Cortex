 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches CORTEX
...
/**
 * Launch CORTEX in the detected terminal emulator.
 *
 ...
 *
 * For pure-argv paths: cortexPath, --prefill, query, cwd travel as distinct
 * argv elements end-to-end. No sh -c. No shellQuote(). The terminal does
 * chdir(cwd) and execvp(cortex, argv). Spaces/quotes/metacharacters in
...
export async function launchInTerminal(
  ...
  const cortexArgs = ['--deep-link-origin']
  if (action.repo) {
    cortexArgs.push('--deep-link-repo', action.repo)
    if (action.lastFetchMs !== undefined) {
      cortexArgs.push('--deep-link-last-fetch', String(action.lastFetchMs))
    }
  }
  if (action.query) {
    cortexArgs.push('--prefill', action.query)
  }

  switch (process.platform) {
    case 'darwin':
      return launchMacosTerminal(terminal, cortexPath, cortexArgs, action.cwd)
    case 'linux':
      return launchLinuxTerminal(terminal, cortexPath, cortexArgs, action.cwd)
    case 'win32':
      return launchWindowsTerminal(terminal, cortexPath, cortexArgs, action.cwd)
    default:
      return false
  }
}
...
async function launchMacosTerminal(
  ...
  switch (terminal.command) {
    ...
    case 'iTerm': {
      const shCmd = buildShellCommand(cortexPath, cortexArgs, cwd)
  ...
  activate
    ...
    case 'Ghostty': {
      ...
      args.push('-e', cortexPath, ...cortexArgs)
    ...
    case 'kitty': {
      ...
      args.push(cortexPath, ...cortexArgs)
      const { code } = await execFileNoThrow('open', args, { useCwd: false })
      if (code === 0) return true
      break
    }
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after cortex exits.
    ...
    case 'PowerShell': {
      ...
      args.push(
        '-NoExit',
        '-Command',
        `${cdCmd}& ${psQuote(cortexPath)} ${cortexArgs.map(psQuote).join(' ')}`,
      )
      break
    }
    ...
    default: {
      ...
      args.push(
        '/k',
        `${cdCmd}${cmdQuote(cortexPath)} ${cortexArgs.map(a => cmdQuote(a)).join(' ')}`,
      )
      break
    }
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[cortexPath, ...cortexArgs].map(shellQuote).join(' ')}`
}
 ...
 * *child process* (cortex.exe) uses CommandLineToArgvW, where a trailing
