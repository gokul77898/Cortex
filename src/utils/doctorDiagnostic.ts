...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      const path = await which('anthropic')
      if (path) {
        return path
      }
    } catch {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/cortex'))
      return join(homedir(), '.local/bin/cortex')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.cortex', 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = ['@anthropic-ai/cortex-code']
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/cortex
    // Linux / macOS have prefix/bin/cortex and prefix/lib/node_modules
    // Windows has prefix/cortex and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'anthropic')
      : join(npmPrefix, 'bin', 'anthropic')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/cortex
      ...
      // If no bin/cortex exists, check for orphaned packages (no bin/cortex symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'anthropic')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'anthropic')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: cortex install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run cortex install to update configuration',
      })
    }
  }
  ...
  const existingAlias = await findCORTEXAlias()
  const validAlias = await findValidCORTEXAlias()

  // Check if running local installation but it's not in PATH
  if (type === 'npm-local') {
    // Check if cortex is already accessible via PATH
    const whichResult = await which('anthropic')
    const cortexInPath = !!whichResult

    // Only show warning if cortex is NOT in PATH AND no valid alias exists
    if (!cortexInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias cortex="~/.cortex/local/cortex"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias cortex="~/.cortex/local/cortex"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @anthropic-ai/cortex-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@anthropic-ai/cortex-code'
        ) {
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `cortex install` for native installation',
      })
    }
  }
