...
export function getSessionEndHookTimeoutMs(): number {
  const raw = process.env.CORTEX_SESSIONEND_HOOKS_TIMEOUT_MS
 ...
 * .cortex/settings.json. This is a defense-in-depth security measure.
 *
 ...
 * (POSIX path conversion, .sh auto-prepend, CORTEX_SHELL_PREFIX).
...
async function execCommandHook(
  ...
  // CORTEX_SHELL_PREFIX wraps the command via POSIX quoting
  ...
  // a CORTEX_PS_SHELL_PREFIX (or shell-aware prefix) is a follow-up.
  const finalCommand =
    !isPowerShell && process.env.CORTEX_SHELL_PREFIX
      ? formatShellPrefixCommand(process.env.CORTEX_SHELL_PREFIX, command)
      : command
 ...
 * .cortex/rules/*.md) is loaded into context. Fire-and-forget — this hook is
 * for observability/audit only and does not support blocking.
 *
 * Dispatch sites:
 * - Eager load at session start (getMemoryFiles in cortexmd.ts)
 * - Eager reload after compaction (getMemoryFiles cache cleared by
 *   runPostCompactCleanup; next call reports load_reason: 'compact')
 * - Lazy load when CORTEX touches a file that triggers nested CLAUDE.md or
