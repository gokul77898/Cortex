 ...
 * Handles conversions between CORTEX's environment and the IDE's environment
 */
...
export interface IDEPathConverter {
  /**
   * Convert path from IDE format to CORTEX's local format
   * Used when reading workspace folders from IDE lockfile
   */
  toLocalPath(idePath: string): string

  /**
   * Convert path from CORTEX's local format to IDE format
...
/**
 * Converter for Windows IDE + WSL CORTEX scenario
 */
