// Scheduled prompts, stored in <project>/.cortex/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.cortex', 'scheduled_tasks.json')
...
/**
 * Read and parse .cortex/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .cortex/scheduled_tasks.json with the given tasks. Creates .cortex/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.cortex'), { recursive: true })
 ...
 * written to .cortex/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while CORTEX
