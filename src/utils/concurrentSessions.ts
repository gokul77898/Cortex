...
function getSessionsDir(): string {
  return join(getCORTEXConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`cortex --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.CORTEX_SESSION_KIND
...
/**
 * True when this REPL is running inside a `cortex --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `cortex ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.CORTEX_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.CORTEX_SESSION_NAME,
              logPath: process.env.CORTEX_SESSION_LOG,
              agent: process.env.CORTEX_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `cortex ps` sparkline
...
/**
 * Push live activity state for `cortex ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See anthropics/cortex-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.cortex/sessions/ is shared with Windows-native CORTEX (symlink
