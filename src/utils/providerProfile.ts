...
export function buildHuggingFaceProfileEnv(options: {
  model?: string | null
  baseUrl?: string | null
  token?: string | null
  ...
  return {
    OPENAI_BASE_URL: options.baseUrl || processEnv.HF_BASE_URL || 'https://api-inference.huggingface.co/v1',
    OPENAI_MODEL: options.model || processEnv.HF_MODEL_ID || 'zai-org/GLM-5',
...
export function hasExplicitProviderSelection(
  ...
  return (
    processEnv.CORTEX_USE_OPENAI !== undefined ||
    processEnv.CORTEX_USE_GITHUB !== undefined ||
    processEnv.CORTEX_USE_GEMINI !== undefined ||
    processEnv.CORTEX_USE_BEDROCK !== undefined ||
    processEnv.CORTEX_USE_VERTEX !== undefined ||
    processEnv.CORTEX_USE_FOUNDRY !== undefined
  )
}
...
export async function buildLaunchEnv(options: {
  ...
  if (options.profile === 'gemini') {
    ...
    delete env.CORTEX_USE_OPENAI
    delete env.CORTEX_USE_GITHUB
  ...
  if (options.profile === 'huggingface') {
    const env: NodeJS.ProcessEnv = {
      ...processEnv,
      CORTEX_USE_OPENAI: '1',
    }
    ...
    env.OPENAI_MODEL =
      persistedEnv.OPENAI_MODEL || processEnv.HF_MODEL_ID || 'zai-org/GLM-5:together'
    env.OPENAI_API_KEY =
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.CORTEX_PROFILE_GOAL),
    processEnv,
