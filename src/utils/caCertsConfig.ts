 ...
 * user-controlled files (~/.cortex/settings.json and ~/.cortex.json),
 ...
 *
 * We read from global config (~/.cortex.json) and user settings
 * (~/.cortex/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.cortex/settings.json),
