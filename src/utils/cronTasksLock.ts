// Scheduler lease lock for .cortex/scheduled_tasks.json.
//
// When multiple CORTEX sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.cortex', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .cortex/ doesn't exist yet — create it and retry once. In steady
