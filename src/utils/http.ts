...
import { getCORTEXCodeUserAgent } from './userAgent.js'
import { getWorkload } from './workloadContext.js'

// WARNING: We rely on `cortex-cli` in the user agent for log filtering.
...
export function getUserAgent(): string {
  ...
  // getCORTEXClient (client.ts:98) calls this per-request inside withRetry,
  // so the read picks up the same setWorkload() value as getAttributionHeader.
  const workload = getWorkload()
  const workloadSuffix = workload ? `, workload/${workload}` : ''
  return `cortex-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.CORTEX_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.CORTEX_ENTRYPOINT) {
    parts.push(process.env.CORTEX_ENTRYPOINT)
  }
  ...
  return `cortex-code/${MACRO.VERSION}${suffix}`
}

// User-Agent for WebFetch requests to arbitrary sites. `CORTEX-User` is
// The first-party provider's publicly documented agent for user-initiated fetches (what site
// operators match in robots.txt); the cortex-code suffix lets them distinguish
// local CLI traffic from anthropic.com server-side fetches.
export function getWebFetchUserAgent(): string {
  const supportUrl =
    getAPIProvider() === 'firstParty'
      ? 'https://support.anthropic.com/'
      : 'https://github.com/Gitlawb/cortex'
  return `CORTEX-User (${getCORTEXCodeUserAgent()}; +${supportUrl})`
}
...
export function getAuthHeaders(): AuthHeaders {
  ...
  // should we try to query keychain / credentials for a valid CORTEX key?
  const apiKey = getCORTEXApiKey()
  if (!apiKey) {
    return {
      headers: {},
