...
export const CLAUDE_IN_CHROME_MCP_SERVER_NAME = 'cortex-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[CORTEX in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isCORTEXInChromeMCPServer(name: string): boolean {
...
export function trackCORTEXInChromeTabId(tabId: number): void {
...
export function isTrackedCORTEXInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[CORTEX in Chrome] No compatible browser found')
    return false
  }
...
export function getSocketDir(): string {
  return `/tmp/cortex-mcp-browser-bridge-${getUsername()}`
}
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- CORTEXForChromeContext.getSocketPaths (external @ant/cortex-for-chrome-mcp) requires a sync () => string[] callback
  ...
  const legacyName = `cortex-mcp-browser-bridge-${getUsername()}`
...
function getSocketName(): string {
  // NOTE: This must match the one used in the CORTEX in Chrome MCP
  return `cortex-mcp-browser-bridge-${getUsername()}`
}
