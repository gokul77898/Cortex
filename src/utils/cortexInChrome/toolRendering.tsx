...
import { trackCORTEXInChromeTabId } from './common.js';
export type { Tool } from '@modelcontextprotocol/sdk/types.js';

/**
 * All tool names from BROWSER_TOOLS in @ant/cortex-for-chrome-mcp.
...
function renderChromeToolUseMessage(input: Record<string, unknown>, toolName: ChromeToolName, verbose: boolean): React.ReactNode {
  ...
  if (typeof tabId === 'number') {
    trackCORTEXInChromeTabId(tabId);
  }
...
/**
 * Renders a clickable "View Tab" link for CORTEX in Chrome MCP tools.
 * Returns null if:
 * - The tool is not a CORTEX in Chrome MCP tool
...
/**
 * Custom tool result message rendering for cortex-in-chrome tools.
...
/**
 * Returns tool method overrides for CORTEX in Chrome MCP tools. Use this to customize
 * rendering for chrome tools in a single spread operation.
 */
export function getCORTEXInChromeMCPToolOverrides(toolName: string): {
  ...
  return {
    userFacingName(_input?: Record<string, unknown>) {
      ...
      return `CORTEX in Chrome[${displayName}]`;
    },
