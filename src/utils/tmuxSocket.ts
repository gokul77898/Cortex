 ...
 * =====================
 * This module manages an isolated tmux socket for CORTEX's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, CORTEX could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started CORTEX from within tmux.
 *
 * HOW IT WORKS:
 * 1. CORTEX creates its own tmux socket: `cortex-<PID>` (e.g., `cortex-12345`)
 ...
 *    (set in Shell.ts via getCORTEXTmuxEnv())
 *
 * This means ANY tmux command run through CORTEX - whether via the Tmux tool
 * directly or via Bash - will operate on CORTEX's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getCORTEXTmuxEnv() returns a value that overrides the
...
const CLAUDE_SOCKET_PREFIX = 'anthropic'
...
/**
 * Gets the socket name for CORTEX's isolated tmux session.
 * Format: cortex-<PID>
 */
export function getCORTEXSocketName(): string {
  if (!socketName) {
 ...
 */
export function getCORTEXSocketPath(): string | null {
  return socketPath
}
 ...
 */
export function setCORTEXSocketInfo(path: string, pid: number): void {
  socketPath = path
  serverPid = pid
}
...
/**
 * Gets the TMUX environment variable value for CORTEX's isolated socket.
 *
 ...
 * initializing the socket. getCORTEXTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Pass CORTEX_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'CORTEX_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set CORTEX_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any CORTEX instance spawned on this socket will inherit this env var,
  ...
  if (pidResult.code === 0) {
    ...
    if (!isNaN(pid)) {
      ...
      setCORTEXSocketInfo(fallbackPath, pid)
      return
    }
    // PID parsing failed
    logForDebugging(
