...
function createHttpsProxyAgent(
  ...
  if (isEnvTruthy(process.env.CORTEX_PROXY_RESOLVES_HOSTS)) {
...
/**
 * Get fetch options for the CORTEX SDK with proxy and mTLS configuration
 * Returns fetch options with appropriate dispatcher for proxy and/or mTLS
 *
 * @param opts.forCORTEXAPI - Enables ANTHROPIC_UNIX_SOCKET tunneling. This
 *   env var is set by `cortex ssh` on the remote CLI to route API calls through
 *   an ssh -R forwarded unix socket to a local auth proxy. It MUST NOT leak
 *   into non-CORTEX-API fetch paths (MCP HTTP/SSE transports, etc.) or those
 *   requests get misrouted to api.anthropic.com. Only the CORTEX SDK client
 *   should pass `true` here.
 */
export function getProxyFetchOptions(opts?: { forCORTEXAPI?: boolean }): {
  tls?: TLSConfig
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `cortex ssh` auth proxy, which
  // hardcodes the upstream to the CORTEX API. Scope to the CORTEX API
  // client so MCP/SSE/other callers don't get their requests misrouted.
  if (opts?.forCORTEXAPI) {
