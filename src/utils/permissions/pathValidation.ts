 ...
 * validation purposes, so commands like `echo foo > /tmp/cortex/x.txt` don't
 * prompt for permission when /tmp/cortex/ is already in the sandbox allowlist.
 *
 * Respects the deny-within-allow list: paths in denyWithinAllow (like
 * .cortex/settings.json) are still blocked even if their parent is in allowOnly.
 */
...
export function isPathAllowed(
  ...
  // Checks: Windows patterns, CORTEX config files, dangerous files (on original + symlink paths)
  if (operationType !== 'read') {
