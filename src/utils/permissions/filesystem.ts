...
import { getCORTEXConfigHomeDir } from '../envUtils.js'
import {
  getFsImplementation,
...
/**
 * If filePath is inside a .cortex/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .cortex/ (settings.json, hooks/, etc.).
 */
export function getCORTEXSkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.cortex', 'skills')),
      prefix: '/.cortex/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.cortex', 'skills')),
      prefix: '~/.cortex/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.cortex/skills/*/**' which matches ALL skills. Return null
...
export function isCORTEXSettingsPath(filePath: string): boolean {
  // SECURITY: Normalize path structure first to prevent bypass via redundant ./
  // sequences like `./.cortex/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.cortex${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.cortex${sep}settings.local.json`)
  ) {
    // Include .cortex/settings.json even for other projects
    return true
  }
...
// Always ask when CORTEX tries to edit its own config files
function isCORTEXConfigFilePath(filePath: string): boolean {
  if (isCORTEXSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .cortex/commands or .cortex/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.cortex', 'commands')
  const agentsDir = join(getOriginalCwd(), '.cortex', 'agents')
  const skillsDir = join(getOriginalCwd(), '.cortex', 'skills')
 ...
 * Path format: ~/.cortex/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for CORTEX to write temporary files.
...
/**
 * Returns the user-specific CORTEX temp directory name.
 * On Unix: 'cortex-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'anthropic' (tmpdir() is already per-user)
 */
export function getCORTEXTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'anthropic'
  }
...
/**
 * Returns the CORTEX temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/cortex-{uid}/ (resolved to /private/tmp/cortex-{uid}/ on macOS)
 * - On Windows: {tmpdir}/cortex/ (e.g., C:\Users\{user}\AppData\Local\Temp\cortex\)
 * This is a per-user temporary directory used by CORTEX for all temp files.
 *
 ...
 * resolution, paths like /tmp/cortex-{uid}/... wouldn't match /private/tmp/cortex-{uid}/...
 */
// Memoized: called per-tool from permission checks (yoloClassifier, sandbox-adapter)
// and per-turn from BashTool prompt. Inputs (CORTEX_TMPDIR env + platform) are
// fixed at startup, and the realpath of the system tmp dir does not change mid-session.
export const getCORTEXTempDir = memoize(function getCORTEXTempDir(): string {
  const baseTmpDir =
    process.env.CORTEX_TMPDIR ||
  ...
  return join(resolvedBaseTmpDir, getCORTEXTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getCORTEXTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/cortex-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getCORTEXTempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/cortex-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/cortex-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .cortex/worktrees/ is a structural path (where CORTEX stores
      // git worktrees), not a user-created dangerous directory. Skip the .cortex
      // segment when it's followed by 'worktrees'. Any nested .cortex directories
 ...
 * - CORTEX config files (.cortex/settings.json, .cortex/commands/, .cortex/agents/)
 * - MCP CLI state files (managed internally by CORTEX)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `CORTEX requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for CORTEX config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isCORTEXConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `CORTEX requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `CORTEX requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .cortex/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `CORTEX requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `CORTEX requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `CORTEX requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `CORTEX requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .cortex is a dangerous directory
  ...
  // 1.6. Check for .cortex/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .cortex/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .cortex/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.cortex) rule in userSettings (e.g. from sandbox
  ...
  // "allow CORTEX to edit its own settings for this session" option actually works.
  const cortexFolderAllowRule = matchingRuleForInput(
    path,
    {
      ...toolPermissionContext,
      alwaysAllowRules: {
  ...
  if (cortexFolderAllowRule) {
    // Check if this rule is scoped under .cortex/ (project or global).
    // Accepts both the broad patterns ('/.cortex/**', '~/.cortex/**') and
    // narrowed ones like '/.cortex/skills/my-skill/**' so users can grant
    ...
    // '/.cortex/../**' from leaking this bypass outside .cortex/.
    const ruleContent = cortexFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      ...
      return {
        ...
        decisionReason: {
          type: 'rule',
          rule: cortexFolderAllowRule,
        },
      }
    }
  }

  // 1.7. Check comprehensive safety validations (Windows patterns, CORTEX config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .cortex/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.cortex/settings.json, .git/, .vscode/, .idea/) falls
    ...
    const skillScope = getCORTEXSkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.cortex/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      const jobsRoot = join(getCORTEXConfigHomeDir(), 'jobs')
      ...
      // the case where ~/.cortex is a symlink (e.g. to /data/cortex-config).
  ...
  // ~/.cortex/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .cortex/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs CORTEX to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .cortex/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .cortex/ only (not ~/.cortex/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.cortex', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/cortex/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.cortex/tasks/) for swarm task coordination
  const tasksDir = join(getCORTEXConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.cortex/teams/) for swarm coordination
  const teamsReadDir = join(getCORTEXConfigHomeDir(), 'teams') + sep
  if (
