...
const RESTORED_KEYS = [
  'CORTEX_PROVIDER_PROFILE_ENV_APPLIED',
  'CORTEX_USE_OPENAI',
  'CORTEX_USE_GEMINI',
  'CORTEX_USE_GITHUB',
  'CORTEX_USE_BEDROCK',
...
describe('applyProviderProfileToProcessEnv', () => {
  test('openai profile clears competing gemini/github flags', async () => {
    ...
    expect(process.env.CORTEX_USE_GEMINI).toBeUndefined()
    expect(process.env.CORTEX_USE_GITHUB).toBeUndefined()
    expect(process.env.CORTEX_USE_OPENAI).toBe('1')
    expect(getAPIProvider()).toBe('openai')
  })

  test('cortex profile clears competing gemini/github flags', async () => {
    process.env.CORTEX_USE_GEMINI = '1'
    process.env.CORTEX_USE_GITHUB = '1'
    const { applyProviderProfileToProcessEnv, getAPIProvider } =
      await importFreshProviderModules()
    ...
    expect(process.env.CORTEX_USE_GEMINI).toBeUndefined()
    expect(process.env.CORTEX_USE_GITHUB).toBeUndefined()
    expect(process.env.CORTEX_USE_OPENAI).toBeUndefined()
...
describe('applyActiveProviderProfileFromConfig', () => {
  ...
  test('does not override explicit startup selection when profile marker is stale', async () => {
    process.env.CORTEX_PROVIDER_PROFILE_ENV_APPLIED = '1'
    process.env.CORTEX_USE_OPENAI = '1'
    process.env.OPENAI_BASE_URL = 'http://localhost:11434/v1'
    process.env.OPENAI_MODEL = 'qwen2.5:3b'
    const { applyActiveProviderProfileFromConfig } =
...
describe('deleteProviderProfile', () => {
  test('deleting final profile clears provider env when active profile applied it', async () => {
    ...
    expect(process.env.CORTEX_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()

    expect(process.env.CORTEX_USE_OPENAI).toBeUndefined()
    expect(process.env.CORTEX_USE_GEMINI).toBeUndefined()
    expect(process.env.CORTEX_USE_GITHUB).toBeUndefined()
    expect(process.env.CORTEX_USE_BEDROCK).toBeUndefined()
    expect(process.env.CORTEX_USE_VERTEX).toBeUndefined()
    expect(process.env.CORTEX_USE_FOUNDRY).toBeUndefined()
  ...
  test('deleting final profile preserves explicit startup provider env', async () => {
    ...
    process.env.CORTEX_PROVIDER_PROFILE_ENV_APPLIED = undefined
    delete process.env.CORTEX_PROVIDER_PROFILE_ENV_APPLIED
    process.env.CORTEX_USE_OPENAI = '1'
    process.env.OPENAI_BASE_URL = 'http://localhost:11434/v1'
    process.env.OPENAI_MODEL = 'qwen2.5:3b'
