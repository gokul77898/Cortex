 ...
 *
 * The parent cortex process keeps these vars (needed for API calls, lazy
...
const GHA_SUBPROCESS_SCRUB = [
  // CORTEX auth — cortex re-reads these per-request, subprocesses don't need them
  'ANTHROPIC_API_KEY',
  'CORTEX_OAUTH_TOKEN',
  ...
  // GitHub Actions OIDC — consumed by the action's JS before cortex spawns;
  ...
  // cortex-code-action-specific duplicates — action JS consumes these during
  // prepare, before spawning cortex. ALL_INPUTS contains cortex_api_key as JSON.
  'ALL_INPUTS',
 ...
 * servers, shell hooks).
 *
 * Gated on CORTEX_SUBPROCESS_ENV_SCRUB. cortex-code-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.CORTEX_SUBPROCESS_ENV_SCRUB)) {
