...
// CORTEX configuration directory names
...
export type CORTEXConfigDirectory = (typeof CLAUDE_CONFIG_DIRECTORIES)[number]
 ...
 * See: https://github.com/anthropics/cortex-code/issues/13893
 *
 ...
 * stopping there makes the parent project's `.cortex/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.cortex/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .cortex directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.cortex/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .cortex/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  subdir: CORTEXConfigDirectory,
  cwd: string,
): string[] {
  ...
  while (true) {
    ...
    const cortexSubdir = join(current, '.cortex', subdir)
    ...
    try {
      statSync(cortexSubdir)
      dirs.push(cortexSubdir)
    } catch (e: unknown) {
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getCORTEXConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.cortex', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .cortex/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .cortex/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .cortex/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (anthropics/cortex-code#29599, #28182, #26992).
    //
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        join(gitRoot, '.cortex', subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainCORTEXSubdir = join(canonicalRoot, '.cortex', subdir)
        if (!projectDirs.includes(mainCORTEXSubdir)) {
          projectDirs.push(mainCORTEXSubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.cortex is
  ...
  (subdir: CORTEXConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
 ...
 * 3. Can be explicitly enabled via CORTEX_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.cortex/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when CORTEX_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.CORTEX_USE_NATIVE_FILE_SEARCH)
