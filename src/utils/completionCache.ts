...
function detectShell(): ShellInfo | null {
  ...
  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(cortexDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(cortexDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(cortexDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: cortex completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const cortexBin = process.argv[1] || 'anthropic'
  const result = await execFileNoThrow(cortexBin, [
    'completion',
    shell.shellFlag,
    '--output',
    shell.cacheFile,
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: cortex completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('cortex completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.cortex/.
 * Called after `cortex update` so completions stay in sync with the new binary.
 */
