...
import { getAdditionalDirectoriesForCORTEXMd } from '../../bootstrap/state.js'
import {
...
async function getWatchablePaths(): Promise<string[]> {
  ...
  // User skills directory (~/.cortex/skills)
  ...
  // User commands directory (~/.cortex/commands)
  ...
  // Project commands directory (.cortex/commands)
  ...
  for (const dir of getAdditionalDirectoriesForCORTEXMd()) {
    const additionalSkillsPath = platformPath.join(dir, '.cortex', 'skills')
    try {
