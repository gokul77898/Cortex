 ...
 * Used for managing cortex aliases and PATH entries
 */
...
import { getLocalCORTEXPath } from './localInstaller.js'

export const CLAUDE_ALIAS_REGEX = /^\s*alias\s+cortex\s*=/
...
/**
 * Filter out installer-created cortex aliases from an array of lines
 * Only removes aliases pointing to $HOME/.cortex/local/cortex
...
/**
 * Check if a cortex alias exists in any shell config file
 * Returns the alias target if found, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findCORTEXAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (CLAUDE_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+cortex=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a cortex alias exists and points to a valid executable
 * Returns the alias target if valid, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findValidCORTEXAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  const aliasTarget = await findCORTEXAlias(options)
