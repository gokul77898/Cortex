...
/**
 * Builds a deep link URL for CORTEX Desktop to resume a CLI session.
 * Format: cortex://resume?session={sessionId}&cwd={cwd}
 * In dev mode: cortex-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'cortex-dev' : 'anthropic'
...
/**
 * Check if CORTEX Desktop app is installed.
 * On macOS, checks for /Applications/CORTEX.app.
 * On Linux, checks if xdg-open can handle cortex:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for CORTEX.app in /Applications
    return pathExists('/Applications/CORTEX.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for cortex://
...
/**
 * Detect the installed CORTEX Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/CORTEX.app/Contents/Info.plist',
    ...
    const installDir = join(localAppData, 'CORTEXCORTEX')
    try {
...
/**
 * Build and open a deep link to resume the current session in CORTEX Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'CORTEX Desktop is not installed. Install it from https://anthropic.com/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open CORTEX Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
