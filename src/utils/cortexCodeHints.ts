/**
 * CORTEX hints protocol.
 *
 * CLIs and SDKs running under CORTEX can emit a self-closing
 * `<cortex-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/cortex-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type CORTEXCodeHintType = 'plugin'

export type CORTEXCodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: CORTEXCodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<cortex-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractCORTEXCodeHints(
  output: string,
  command: string,
): { hints: CORTEXCodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<cortex-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: CORTEXCodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[cortexCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[cortexCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[cortexCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as CORTEXCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: CORTEXCodeHint | null = null
...
export function setPendingHint(hint: CORTEXCodeHint): void {
...
export function getPendingHintSnapshot(): CORTEXCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetCORTEXCodeHintStore(): void {
  pendingHint = null
