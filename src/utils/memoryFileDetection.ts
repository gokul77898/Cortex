...
/**
 * Detects if a file path is a session-related file under ~/.cortex.
...
/**
 * Check if a file is a CORTEX-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: CLAUDE.md, CLAUDE.local.md, .cortex/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  const configDirCmp = toComparable(getCORTEXConfigHomeDir())
...
// Excludes CLAUDE.md, CLAUDE.local.md, .cortex/rules/ (user-managed).
