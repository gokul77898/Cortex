...
import { queryHaiku } from '../services/api/cortex.js';
...
import { checkAndRefreshOAuthTokenIfNeeded, getCORTEXAIOAuthTokens } from './auth.js';
...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "cortex/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "cortex/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "cortex/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "cortex/improve-data-processing"}
...
/**
 * Generates a title and branch name for a coding session using CORTEX Haiku
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'cortex/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    const accessToken = getCORTEXAIOAuthTokens()?.accessToken;
    if (!accessToken) {
      ...
      throw new Error('CORTEX web sessions require authentication with a CORTEX.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run cortex --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run cortex --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run cortex --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run cortex --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote CORTEX.ai session with error handling and UI feedback.
...
export async function pollRemoteSessionEvents(sessionId: string, afterId: string | null = null, opts?: {
  ...
  const headers = {
    ...
    'cortex-beta': 'ccr-byoc-2025-07-29',
...
/**
 * Creates a remote CORTEX.ai session using the Sessions API.
 *
 * Two source modes:
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, CORTEX_OAUTH_TOKEN is auto-injected from the
   ...
   * a new cortex/ branch. Sets allow_unrestricted_git_push on the source and
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        CORTEX_OAUTH_TOKEN: accessToken,
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please setup GitHub on https://anthropic.com/code' : '';
        let msg: string;
    ...
    // Select environment based on settings, then cortex_cloud preference, then first available.
    // Prefer cortex_cloud environments over byoc: cortex_cloud environments (e.g. "Default")
    ...
    let cloudEnv = environments.find(env => env.kind === 'cortex_cloud');
    ...
    if (options.useDefaultEnvironment && !cloudEnv) {
      logForDebugging(`No cortex_cloud in env list (${environments.length} envs); retrying fetchEnvironments`);
      const retried = await fetchEnvironments();
      cloudEnv = retried?.find(env => env.kind === 'cortex_cloud');
      if (!cloudEnv) {
        logError(new Error(`No cortex_cloud environment available after retry (got: ${(retried ?? environments).map(e => `${e.name} (${e.kind})`).join(', ')}). Silent byoc fallthrough would launch into a dead env — fail fast instead.`));
        return null;
      }
