...
/**
 * `cortex ssh` remote: ANTHROPIC_UNIX_SOCKET routes auth through a -R forwarded
 * socket to a local proxy, and the launcher sets a handful of placeholder auth
 * env vars that the remote's ~/.cortex settings.env MUST NOT clobber (see
 * isCORTEXAuthEnabled). Strip them from any settings-sourced env object.
 */
 ...
 * CORTEX_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.cortex/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.CORTEX_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.cortex/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.cortex.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.cortex/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads CORTEX_USE_BEDROCK,
  ...
  // when CORTEX_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * CORTEX_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
