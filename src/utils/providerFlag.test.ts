...
describe('applyProviderFlag - cortex', () => {
  test('sets no env vars for cortex (default)', () => {
...
describe('applyProviderFlag - gemini', () => {
  test('sets CORTEX_USE_GEMINI=1', () => {
    const result = applyProviderFlag('gemini', [])
    expect(result.error).toBeUndefined()
    expect(process.env.CORTEX_USE_GEMINI).toBe('1')
  })
...
describe('applyProviderFlag - github', () => {
  test('sets CORTEX_USE_GITHUB=1', () => {
    const result = applyProviderFlag('github', [])
    expect(result.error).toBeUndefined()
    expect(process.env.CORTEX_USE_GITHUB).toBe('1')
  })
})

describe('applyProviderFlag - bedrock', () => {
  test('sets CORTEX_USE_BEDROCK=1', () => {
    const result = applyProviderFlag('bedrock', [])
    expect(result.error).toBeUndefined()
    expect(process.env.CORTEX_USE_BEDROCK).toBe('1')
  })
})

describe('applyProviderFlag - vertex', () => {
  test('sets CORTEX_USE_VERTEX=1', () => {
    const result = applyProviderFlag('vertex', [])
    expect(result.error).toBeUndefined()
    expect(process.env.CORTEX_USE_VERTEX).toBe('1')
  })
})

describe('applyProviderFlag - ollama', () => {
  test('sets CORTEX_USE_OPENAI=1 with Ollama base URL', () => {
