...
/**
 * Generates CORTEX specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getCORTEXCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in CORTEX's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and CORTEX content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const cortexCodeContent = await getCORTEXCodeSnapshotContent()
 ...
 *
 * This function is a critical part of CORTEX CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows CORTEX CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with CORTEX defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getCORTEXConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.CORTEX_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - CORTEX home: ${getCORTEXConfigHomeDir()}`)
