...
export function getProjectsDir(): string {
  return join(getCORTEXConfigHomeDir(), 'projects')
}
...
function getEntrypoint(): string | undefined {
  return process.env.CORTEX_ENTRYPOINT
}
...
class Project {
   ...
   * CORTEX_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.CORTEX_SKIP_PROMPT_HISTORY)
    )
  }
 ...
 *
 * Streaming (cortex.ts:~2024) emits one AssistantMessage per content_block_stop
...
/**
 * Append a periodic task summary for `cortex ps`. Unlike ai-title this is
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.CORTEX_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // CORTEX_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.CORTEX_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.CORTEX_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
