 ...
 * pipeline from user input to first token arrival. Enable by setting CORTEX_PROFILE_QUERY=1
 *
 ...
 * - query_client_creation_start/end: Creating CORTEX client
...
const ENABLED = isEnvTruthy(process.env.CORTEX_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set CORTEX_PROFILE_QUERY=1)'
  }
