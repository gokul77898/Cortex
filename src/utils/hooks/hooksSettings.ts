...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.cortex/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.cortex/settings.json)'
    case 'projectSettings':
      return 'Project settings (.cortex/settings.json)'
    case 'localSettings':
      return 'Local settings (.cortex/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.cortex/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.cortex/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by CORTEX)'
    default:
