...
import { accumulateUsage, updateUsage } from '../services/api/cortex.js'
 ...
 * to share the parent's prompt cache. The CORTEX API cache key is composed of:
 ...
 * cortex.ts (but only for older models that do not use adaptive thinking).
...
export type ForkedAgentParams = {
   ...
   * AND budget_tokens (via clamping in cortex.ts). If the fork uses cacheSafeParams
...
export async function runForkedAgent({
  ...
  // tool_uses are repaired downstream by ensureToolResultPairing in cortex.ts,
