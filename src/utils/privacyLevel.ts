 ...
 *   CORTEX_DISABLE_NONESSENTIAL_TRAFFIC  →  essential-traffic
...
export function getPrivacyLevel(): PrivacyLevel {
  if (process.env.CORTEX_DISABLE_NONESSENTIAL_TRAFFIC) {
 ...
 * Equivalent to the old `process.env.CORTEX_DISABLE_NONESSENTIAL_TRAFFIC` check.
 */
...
export function getEssentialTrafficOnlyReason(): string | null {
  if (process.env.CORTEX_DISABLE_NONESSENTIAL_TRAFFIC) {
    return 'CORTEX_DISABLE_NONESSENTIAL_TRAFFIC'
  }
  return null
}
