...
const PROFILE_ENV_APPLIED_FLAG = 'CORTEX_PROVIDER_PROFILE_ENV_APPLIED'
  ...
  switch (preset) {
    case 'anthropic':
      return {
        ...
        model: process.env.ANTHROPIC_MODEL ?? 'cortex-sonnet-4-6',
...
export function clearProviderProfileEnvFromProcessEnv(
  ...
  delete processEnv.CORTEX_USE_OPENAI
  delete processEnv.CORTEX_USE_GEMINI
  delete processEnv.CORTEX_USE_GITHUB
  delete processEnv.CORTEX_USE_BEDROCK
  delete processEnv.CORTEX_USE_VERTEX
  delete processEnv.CORTEX_USE_FOUNDRY
