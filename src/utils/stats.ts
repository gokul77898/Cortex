...
/**
 * Convert a PersistedStatsCache to CORTEXCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all CORTEX sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateCORTEXCodeStats(): Promise<CORTEXCodeStats> {
 ...
 */
export async function aggregateCORTEXCodeStatsForRange(
  range: StatsDateRange,
): Promise<CORTEXCodeStats> {
  if (range === 'all') {
    return aggregateCORTEXCodeStats()
  }
  ...
  return processedStatsToCORTEXCodeStats(stats)
}

/**
 * Convert ProcessedStats to CORTEXCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToCORTEXCodeStats(
...
function getEmptyStats(): CORTEXCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
