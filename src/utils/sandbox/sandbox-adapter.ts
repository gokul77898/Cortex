/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with CORTEX CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and CORTEX CLI's
...
import { getCORTEXTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve CORTEX-specific path patterns for sandbox-runtime.
 *
 * CORTEX uses special path prefixes in permission rules:
...
/**
 * Convert CORTEX settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and CORTEX temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getCORTEXTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where CORTEX started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.cortex', 'settings.json'))
    denyWrite.push(resolve(cwd, '.cortex', 'settings.local.json'))
  }

  // Block writes to .cortex/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .cortex/commands and
  // .cortex/agents but not .cortex/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full CORTEX capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.cortex', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.cortex', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when CORTEX's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See anthropics/cortex-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * CORTEX's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. anthropics/cortex-code#29316.
 */
 ...
 * This is a CORTEX CLI-specific function that updates local settings.
 */
...
/**
 * CORTEX CLI sandbox manager - wraps sandbox-runtime with CORTEX-specific features
 */
