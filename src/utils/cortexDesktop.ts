...
export async function getCORTEXDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - CORTEX Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/CORTEX/cortex_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for CORTEX Desktop config in each user directory
  ...
  throw new Error(
    'Could not find CORTEX Desktop config file in Windows. Make sure CORTEX Desktop is installed on Windows.',
  )
}

export async function readCORTEXDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - CORTEX Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getCORTEXDesktopConfigPath()
