 ...
 * one into the other. Surfaced as warnings by `cortex plugin validate` since
...
function detectManifestType(
  ...
  // Check if it's in .cortex-plugin directory
  if (dirName === '.cortex-plugin') {
 ...
 * '..' a user added to "climb out of .cortex-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .cortex-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .cortex-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .cortex-plugin/), not relative to marketplace.json. ' +
...
export async function validatePluginManifest(
  ...
  // them. `cortex plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — CORTEX ` +
  ...
  if (result.success) {
    ...
    // but the CORTEX.ai marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. CORTEX accepts ` +
          `it, but the CORTEX.ai marketplace sync requires kebab-case ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.cortex-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.cortex-plugin/plugin.json says "${manifestVersion}". ` +
 ...
 * for the load path, but authors running `cortex plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and CORTEX ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .cortex-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.cortex-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .cortex-plugin/marketplace.json or .cortex-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
