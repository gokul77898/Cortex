 ...
 *
 * getOriginalCwd() (not getCwd()) because "current project" is where CORTEX
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.cortex-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          if (dirEntries.includes('.cortex-plugin')) {
