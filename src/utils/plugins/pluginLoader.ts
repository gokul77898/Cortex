 ...
 *
 * This module is responsible for discovering, loading, and validating CORTEX plugins
 ...
 * Format: ~/.cortex/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.cortex/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.CORTEX_REMOTE)
 ...
 * CORTEX_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.cortex-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".cortex-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".cortex-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.cortex-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".cortex-plugin/name")
 ...
 *
 * CORTEX_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getCORTEXCodeMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .cortex-plugin/ directory (for export)
