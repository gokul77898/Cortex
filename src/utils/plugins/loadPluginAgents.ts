...
async function loadAgentFromFile(
  ...
  try {
    ...
    // time. For this level of control, define the agent in .cortex/agents/
    ...
    for (const field of ['permissionMode', 'hooks', 'mcpServers'] as const) {
      if (frontmatter[field] !== undefined) {
        logForDebugging(
          `Plugin agent file ${filePath} sets ${field}, which is ignored for plugin agents. Use .cortex/agents/ for this level of control.`,
          { level: 'warn' },
        )
      }
    }
