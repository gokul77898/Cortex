...
const PLUGIN_ID_HASH_SALT = 'cortex-plugin-telemetry-v1'
 ...
 * marketplace-origin.
 *
 * - official: from an allowlisted CORTEX marketplace
...
export function buildPluginTelemetryFields(
  ...
  // Both official marketplaces and builtin plugins are CORTEX-controlled
  // — safe to expose real names in the redacted columns.
  const isCORTEXControlled =
  ...
  return {
    ...
    plugin_name_redacted: (isCORTEXControlled
      ? name
      : 'third-party') as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
    marketplace_name_redacted: (isCORTEXControlled && marketplace
      ? marketplace
      : 'third-party') as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
    is_official_plugin: isCORTEXControlled,
  }
}
