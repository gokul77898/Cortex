/**
 * Perfetto Tracing for CORTEX (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via CORTEX_PERFETTO_TRACE=1 or CORTEX_PERFETTO_TRACE=<path>
 * 2. Optionally set CORTEX_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run CORTEX normally
 * 4. Trace file is written to ~/.cortex/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.CORTEX_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    if (isEnvTruthy(envValue)) {
      const tracesDir = join(getCORTEXConfigHomeDir(), 'traces')
    ...
    // Start periodic full-trace write if CORTEX_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.CORTEX_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
