 ...
 * The cortexs and cortex-experimental orgs contain PUBLIC repos
 * (e.g. anthropics/cortex-code, cortex-experimental/sandbox-runtime).
...
const INTERNAL_MODEL_REPOS = [
  'github.com:anthropics/cortex-cli-internal',
  'github.com/anthropics/cortex-cli-internal',
  'github.com:anthropics/cortex',
  'github.com/anthropics/cortex',
  ...
  'github.com/anthropics/trellis',
  'github.com:anthropics/cortex-for-hiring',
  'github.com/anthropics/cortex-for-hiring',
...
export function sanitizeModelName(shortName: string): string {
  ...
  if (shortName.includes('opus-4-6')) return 'cortex-opus-4-6'
  if (shortName.includes('opus-4-5')) return 'cortex-opus-4-5'
  if (shortName.includes('opus-4-1')) return 'cortex-opus-4-1'
  if (shortName.includes('opus-4')) return 'cortex-opus-4'
  if (shortName.includes('sonnet-4-6')) return 'cortex-sonnet-4-6'
  if (shortName.includes('sonnet-4-5')) return 'cortex-sonnet-4-5'
  if (shortName.includes('sonnet-4')) return 'cortex-sonnet-4'
  if (shortName.includes('sonnet-3-7')) return 'cortex-sonnet-3-7'
  if (shortName.includes('haiku-4-5')) return 'cortex-haiku-4-5'
  if (shortName.includes('haiku-3-5')) return 'cortex-haiku-3-5'
  // Unknown models get a generic name
  return 'anthropic'
}

/**
 * Attribution state for tracking CORTEX's contributions to files.
 */
...
/**
 * Summary of CORTEX's contribution for a commit.
 */
export type AttributionSummary = {
  cortexPercent: number
  cortexChars: number
  humanChars: number
  surfaces: string[]
}
...
export type AttributionData = {
  ...
  surfaceBreakdown: Record<string, { cortexChars: number; percent: number }>
...
export function getClientSurface(): string {
  return process.env.CORTEX_ENTRYPOINT ?? 'cli'
}

/**
 * Build a surface key that includes the model name.
 * Format: "surface/model" (e.g., "cli/cortex-sonnet")
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate CORTEX's character contribution
    let cortexContribution: number
    ...
    if (oldContent === '' || newContent === '') {
      ...
      cortexContribution = Math.max(oldChangedLen, newChangedLen)
    }

    // Get current file state if it exists
    const existingState = existingFileStates.get(normalizedPath)
    const existingContribution = existingState?.cortexContribution ?? 0

    return {
      contentHash: computeContentHash(newContent),
      cortexContribution: existingContribution + cortexContribution,
      mtime,
    }
  } catch (error) {
...
/**
 * Track a file modification by CORTEX.
...
export function trackFileModification(
  ...
  logForDebugging(
    `Attribution: Tracked ${newFileState.cortexContribution} chars for ${normalizedPath}`,
  )
...
/**
 * Track a file creation by CORTEX (e.g., via bash command).
 * Used when CORTEX creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by CORTEX (e.g., via bash rm command).
 * Used when CORTEX deletes a file through a non-tracked mechanism.
 */
export function trackFileDeletion(
  ...
  const newFileState: FileAttributionState = {
    ...
    cortexContribution: existingContribution + deletedChars,
    mtime: Date.now(),
  }
  ...
  logForDebugging(
    `Attribution: Tracked deletion of ${normalizedPath} (${deletedChars} chars removed, total contribution: ${newFileState.cortexContribution})`,
  )
...
export async function calculateCommitAttribution(
    ...
    for (const [path, fileState] of fileStates) {
      ...
      if (existing) {
        mergedFileStates.set(path, {
          ...fileState,
          cortexContribution:
            existing.cortexContribution + fileState.cortexContribution,
        })
      } else {
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // CORTEX deleted this file (tracked deletion)
          cortexChars = fileState.cortexContribution
          humanChars = 0
        } else {
        ...
        try {
          ...
          if (fileState) {
            ...
            // New file not created by CORTEX
      ...
      cortexChars = Math.max(0, cortexChars)
      humanChars = Math.max(0, humanChars)

      const total = cortexChars + humanChars
      const percent = total > 0 ? Math.round((cortexChars / total) * 100) : 0
  ...
  for (const result of fileResults) {
    ...
    files[result.file] = {
      cortexChars: result.cortexChars,
      humanChars: result.humanChars,
      percent: result.percent,
      surface: result.surface,
    }

    totalCORTEXChars += result.cortexChars
    totalHumanChars += result.humanChars

    surfaceCounts[result.surface] =
      (surfaceCounts[result.surface] ?? 0) + result.cortexChars
  }

  const totalChars = totalCORTEXChars + totalHumanChars
  const cortexPercent =
    totalChars > 0 ? Math.round((totalCORTEXChars / totalChars) * 100) : 0

  // Calculate surface breakdown (percentage of total content per surface)
  const surfaceBreakdown: Record<
    string,
    { cortexChars: number; percent: number }
  > = {}
  for (const [surface, chars] of Object.entries(surfaceCounts)) {
    ...
    surfaceBreakdown[surface] = { cortexChars: chars, percent }
  }
