...
import { getMaxOutputTokensForModel } from '../services/api/cortex.ts'
import {
...
const originalEnv = {
  ...
  CORTEX_MAX_OUTPUT_TOKENS: process.env.CORTEX_MAX_OUTPUT_TOKENS,
}

afterEach(() => {
  if (originalEnv.CORTEX_USE_OPENAI === undefined) {
    delete process.env.CORTEX_USE_OPENAI
  } else {
    process.env.CORTEX_USE_OPENAI = originalEnv.CORTEX_USE_OPENAI
  }
  if (originalEnv.CORTEX_MAX_OUTPUT_TOKENS === undefined) {
    delete process.env.CORTEX_MAX_OUTPUT_TOKENS
  } else {
    process.env.CORTEX_MAX_OUTPUT_TOKENS =
      originalEnv.CORTEX_MAX_OUTPUT_TOKENS
  }
})
...
test('deepseek-chat clamps oversized max output overrides to the provider limit', () => {
  process.env.CORTEX_USE_OPENAI = '1'
  process.env.CORTEX_MAX_OUTPUT_TOKENS = '32000'
...
test('gpt-5.4 family keeps large max output overrides within provider limits', () => {
  process.env.CORTEX_USE_OPENAI = '1'
  process.env.CORTEX_MAX_OUTPUT_TOKENS = '200000'
