...
import { getLargeMemoryFiles, MAX_MEMORY_CHARACTER_COUNT, type MemoryFileInfo } from './cortexmd.js';
import figures from 'figures';
import { getCwd } from './cwd.js';
import { relative } from 'path';
import { formatNumber } from './format.js';
import type { getGlobalConfig } from './config.js';
import { getCORTEXApiKeyWithSource, getApiKeyFromConfigOrMacOSKeychain, getAuthTokenSource, isCORTEXAISubscriber } from './auth.js';
...
const cortexAiSubscriberExternalTokenNotice: StatusNoticeDefinition = {
  id: 'cortex-ai-external-token',
  type: 'warning',
  isActive: () => {
    const authTokenInfo = getAuthTokenSource();
    return isCORTEXAISubscriber() && (authTokenInfo.source === 'ANTHROPIC_AUTH_TOKEN' || authTokenInfo.source === 'apiKeyHelper');
  },
  render: () => {
    const authTokenInfo = getAuthTokenSource();
    return <Box flexDirection="row" marginTop={1}>
        <Text color="warning">{figures.warning}</Text>
        <Text color="warning">
          Auth conflict: Using {authTokenInfo.source} instead of CORTEX account
...
const apiKeyConflictNotice: StatusNoticeDefinition = {
  ...
  render: () => {
    ...
    return <Box flexDirection="row" marginTop={1}>
        <Text color="warning">{figures.warning}</Text>
        <Text color="warning">
          Auth conflict: Using {apiKeySource} instead of CORTEX Console key.
          Either unset {apiKeySource}, or run `cortex /logout`.
        </Text>
      </Box>;
  }
};
  ...
  render: () => {
    ...
    return <Box flexDirection="column" marginTop={1}>
        ...
        <Box flexDirection="column" marginLeft={3}>
          <Text color="warning">
            · Trying to use{' '}
            {authTokenInfo.source === 'anthropic.com' ? 'anthropic.com' : authTokenInfo.source}
            ?{' '}
            {apiKeySource === 'ANTHROPIC_API_KEY' ? 'Unset the ANTHROPIC_API_KEY environment variable, or cortex /logout then say "No" to the API key approval before login.' : apiKeySource === 'apiKeyHelper' ? 'Unset the apiKeyHelper setting.' : 'cortex /logout'}
          </Text>
          <Text color="warning">
            · Trying to use {apiKeySource}?{' '}
            {authTokenInfo.source === 'anthropic.com' ? 'cortex /logout to sign out of anthropic.com.' : `Unset the ${authTokenInfo.source} environment variable.`}
          </Text>
        </Box>
      </Box>;
  }
  ...
  render: () => {
    ...
    return <Box flexDirection="row" gap={1} marginLeft={1}>
        ...
        <Text>
          ...
          <Text bold>https://docs.anthropic.com/s/cortex-code-jetbrains</Text>
        </Text>
      </Box>;
  }
};
...
export const statusNoticeDefinitions: StatusNoticeDefinition[] = [largeMemoryFilesNotice, largeAgentDescriptionsNotice, cortexAiSubscriberExternalTokenNotice, apiKeyConflictNotice, bothAuthMethodsNotice, jetbrainsPluginNotice];
