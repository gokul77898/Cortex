...
function getLocalInstallDir(): string {
  return join(getCORTEXConfigHomeDir(), 'local')
}
export function getLocalCORTEXPath(): string {
  return join(getLocalInstallDir(), 'anthropic')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.cortex/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'cortex-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'anthropic')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/cortex" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update CORTEX CLI package in the local directory
 ...
 */
export async function installOrUpdateCORTEXPackage(
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install CORTEX CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'anthropic'))
    return true
  } catch {
    return false
  }
