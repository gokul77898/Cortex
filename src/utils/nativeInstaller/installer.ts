...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'cortex.exe' : 'anthropic'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'anthropic', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'anthropic', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'anthropic', 'locks'),
...
async function isPossibleCORTEXBinary(filePath: string): Promise<boolean> {
  try {
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('cortex-cli-native-'),
    )
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/cortex to the version binary
  ...
  if (!(await isPossibleCORTEXBinary(executablePath))) {
...
async function versionIsAvailable(version: string): Promise<boolean> {
  ...
  return isPossibleCORTEXBinary(installPath)
}
...
async function updateLatest(
  ...
  if (
    ...
    (await isPossibleCORTEXBinary(executablePath))
  ) {
...
export async function checkInstall(
  ...
  // Check if cortex executable exists and is valid.
  ...
  // isPossibleCORTEXBinary stats the path internally, so we don't pre-check
  // with access() — that would be a TOCTOU between access and the stat.
  if (isWindows) {
    // On Windows it's a copied executable, not a symlink
    if (!(await isPossibleCORTEXBinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but cortex command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleCORTEXBinary(absoluteTarget))) {
        messages.push({
          message: `CORTEX symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but cortex command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleCORTEXBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid CORTEX binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^cortex\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the cortex symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed cortex symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove cortex symlink: ${error}`))
  }
}

/**
 * Clean up old cortex aliases from shell configuration files
...
export async function cleanupShellAliases(): Promise<SetupMessage[]> {
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      const { filtered, hadAlias } = filterCORTEXAliases(lines)

      if (hadAlias) {
        await writeFileLines(configFile, filtered)
        messages.push({
          message: `Removed cortex alias from ${configFile}. Run: unalias cortex`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up cortex alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'cortex.cmd')
      const binPs1 = join(globalPrefix, 'cortex.ps1')
      const binExe = join(globalPrefix, 'anthropic')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'anthropic')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @anthropic-ai/cortex-code
  const codePackageResult = await attemptNpmUninstall(
    '@anthropic-ai/cortex-code',
  )
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@anthropic-ai/cortex-code') {
  ...
  // Check for local installation at ~/.cortex/local
  const localInstallDir = join(homedir(), '.cortex', 'local')
