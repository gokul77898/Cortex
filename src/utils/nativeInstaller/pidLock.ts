 ...
 *
 * This module provides PID-based locking for running CORTEX versions.
...
/**
 * Validate that a running process is actually a CORTEX process
 * This helps mitigate PID reuse issues
 */
function isCORTEXProcess(pid: number, expectedExecPath: string): boolean {
  if (!isProcessRunning(pid)) {
    return false
  }

  // If the PID matches our current process, we know it's valid
  // This handles test environments where the command might not contain 'anthropic'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'anthropic' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('anthropic') ||
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a CORTEX process?
  // This helps with PID reuse scenarios
  if (!isCORTEXProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be CORTEX - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a CORTEX process (to handle PID reuse scenarios)
