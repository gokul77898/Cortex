...
function cleanupTerminalModes(skipUnmount: boolean = false): void {
  ...
  try {
    ...
    // Respect CORTEX_DISABLE_TERMINAL_TITLE — if the user opted out of
...
function printResumeHint(): void {
  ...
  if (
    ...
    try {
      ...
      // Don't show resume hint if no session file exists (e.g., subcommands like `cortex update`)
...
export async function gracefulShutdown(
  ...
  // overall execution via a single budget (CORTEX_SESSIONEND_HOOKS_TIMEOUT_MS,
