 ...
 * - Checks environment kind (CORTEX_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires CORTEX_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.CORTEX_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but CORTEX_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
