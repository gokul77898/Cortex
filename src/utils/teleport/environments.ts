...
export type EnvironmentKind = 'cortex_cloud' | 'byoc' | 'bridge'
...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'CORTEX web sessions require authentication with a CORTEX.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
...
/**
 * Creates a default cortex_cloud environment for users who have none.
