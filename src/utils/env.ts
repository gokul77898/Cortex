...
export const getGlobalCORTEXFile = memoize((): string => {
  // Legacy fallback for backwards compatibility
  if (
    getFsImplementation().existsSync(
      join(getCORTEXConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getCORTEXConfigHomeDir(), '.config.json')
  }

  const filename = `.cortex${fileSuffixForOauthConfig()}.json`
 ...
 * If CORTEX_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.CORTEX_HOST_PLATFORM
