...
import { getAPIMetadata } from '../services/api/cortex.js'
import { getCORTEXClient } from '../services/api/client.js'
import { getModelBetas, modelSupportsStructuredOutputs } from './betas.js'
import { computeFingerprint } from './fingerprint.js'
import { normalizeModelStringForAPI } from './model/model.js'

type MessageParam = CORTEX.MessageParam
type TextBlockParam = CORTEX.TextBlockParam
type Tool = CORTEX.Tool
type ToolChoice = CORTEX.ToolChoice
type BetaMessage = CORTEX.Beta.Messages.BetaMessage
type BetaJSONOutputFormat = CORTEX.Beta.Messages.BetaJSONOutputFormat
type BetaThinkingConfigParam = CORTEX.Beta.Messages.BetaThinkingConfigParam
...
export type SideQueryOptions = {
  ...
  /** Abort signal */
  signal?: AbortSignal
  /** Skip CLI system prompt prefix (keeps attribution header for OAuth). Default true — side queries are internal classifiers with their own prompt. Set false only for queries that need the full "You are CORTEX…" prefix. */
...
export async function sideQuery(opts: SideQueryOptions): Promise<BetaMessage> {
  ...
  const client = await getCORTEXClient({
    maxRetries,
    model,
    source: 'side_query',
  })
