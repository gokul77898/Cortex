/**
 * MDM (Mobile Device Management) profile enforcement for CORTEX managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.cortex.cortexcode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\CORTEXCode` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\CORTEXCode` (user-writable, lowest priority)
 * - Linux: No MDM equivalent (uses /etc/cortex-code/managed-settings.json instead)
 *
