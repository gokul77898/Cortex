...
/**
 * CORTEX parses history for pasted content references to match back to
...
async function* makeLogEntryReader(): AsyncGenerator<LogEntry> {
  ...
  const historyPath = join(getCORTEXConfigHomeDir(), 'history.jsonl')
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by CORTEX's Tungsten tool.
  // This prevents verification/test sessions from polluting the user's real command history.
  if (isEnvTruthy(process.env.CORTEX_SKIP_PROMPT_HISTORY)) {
    return
  }
