...
import { getRateLimitTier, getSubscriptionType, isCORTEXAISubscriber } from 'src/utils/auth.js';
...
export function RateLimitMessage(t0) {
  ...
  const isMax20x = rateLimitTier === "default_cortex_max_20x";
  let t3;
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = shouldProcessMockLimits() || isCORTEXAISubscriber();
    $[2] = t3;
  } else {
    t3 = $[2];
  }
  ...
  const isCurrentlyRateLimited = cortexAiLimits.status === "rejected" && cortexAiLimits.resetsAt !== undefined && !cortexAiLimits.isUsingOverage;
  ...
  bb0: {
    ...
    if ($[7] !== shouldAutoOpenRateLimitOptionsMenu) {
      t7 = getUpsellMessage({
        ...
        isTeamOrEnterprise,
        hasBillingAccess: hasCORTEXAiBillingAccess()
      });
