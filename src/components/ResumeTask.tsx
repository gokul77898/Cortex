  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading CORTEX sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your CORTEX sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
...
function renderErrorSpecificGuidance(errorType: LoadErrorType): React.ReactNode {
  switch (errorType) {
    ...
    case 'auth':
      return <Box marginY={1} flexDirection="column">
          <Text dimColor>Teleport requires a CORTEX account</Text>
          <Text dimColor>
            Run <Text bold>/login</Text> and select &quot;CORTEX account with
            subscription&quot;
          </Text>
        </Box>;
    case 'api':
      return <Box marginY={1} flexDirection="column">
          <Text dimColor>Sorry, CORTEX encountered an error</Text>
        </Box>;
    case 'other':
