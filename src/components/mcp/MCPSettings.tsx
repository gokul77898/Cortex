...
import { CORTEXAuthProvider } from '../../services/mcp/auth.js';
import type { McpCORTEXAIProxyServerConfig, McpHTTPServerConfig, McpSSEServerConfig, McpStdioServerConfig } from '../../services/mcp/types.js';
    ...
    t5 = () => {
      ...
      const prepareServers = async function prepareServers() {
        const serverInfos = await Promise.all(filteredClients.map(async client_0 => {
          ...
          const isCORTEXAIProxy = client_0.config.type === "cortexai-proxy";
          let isAuthenticated = undefined;
          if (isSSE || isHTTP) {
            const authProvider = new CORTEXAuthProvider(client_0.name, client_0.config as McpSSEServerConfig | McpHTTPServerConfig);
          ...
          if (isCORTEXAIProxy) {
            return {
              ...baseInfo,
              transport: "cortexai-proxy" as const,
              isAuthenticated: false,
              config: client_0.config as McpCORTEXAIProxyServerConfig
            };
          } else {
            if (isSSE) {
              return {
  ...
  if ($[10] !== agentMcpServers.length || $[11] !== filteredClients.length || $[12] !== onComplete || $[13] !== servers.length) {
    t7 = () => {
      ...
      if (servers.length === 0 && agentMcpServers.length === 0) {
        onComplete("No MCP servers configured. Please run /doctor if this is unexpected. Otherwise, run `cortex mcp --help` or visit https://code.anthropic.com/docs/en/mcp to learn more.");
      }
    };
    ...
    case "server-menu":
      {
        ...
        const defaultTab = viewState.server.transport === "cortexai-proxy" ? "anthropic.com" : "CORTEX";
