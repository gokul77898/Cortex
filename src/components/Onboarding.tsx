...
import { isCORTEXAuthEnabled } from '../utils/auth.js';
...
export function Onboarding({
  ...
  const [oauthEnabled] = useState(() => isCORTEXAuthEnabled());
  ...
  const securityStep = <Box flexDirection="column" gap={1} paddingLeft={1}>
      ...
      <Box flexDirection="column" width={70}>
        ...
        <OrderedList>
          <OrderedList.Item>
            <Text>CORTEX can make mistakes</Text>
            <Text dimColor wrap="wrap">
              You should always review CORTEX&apos;s responses, especially when
              <Newline />
              running code.
              <Newline />
            </Text>
  ...
  const apiKeyNeedingApproval = useMemo(() => {
    ...
    if (!process.env.ANTHROPIC_API_KEY || isRunningOnHomespace() || !isCORTEXAuthEnabled()) {
      return '';
    }
  ...
  if (shouldOfferTerminalSetup()) {
    steps.push({
      ...
      component: <Box flexDirection="column" gap={1} paddingLeft={1}>
          <Text bold>Use CORTEX&apos;s terminal setup?</Text>
