 ...
 *
 * Used by both `cortex remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getCORTEXAIOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getCORTEXAIOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getCORTEXAIOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `cortex remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on anthropic.com/code.
 *
