...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with anthropic.com subscriptions. Please use `/login` to sign in with your anthropic.com account.'

/** Full error printed when `cortex remote-control` is run without auth. */
...
export type WorkSecret = {
  ...
  cortex_code_args?: Record<string, string> | null
...
/**
 * How `cortex remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so anthropic.com can filter
 ...
 */
export type BridgeWorkerType = 'cortex_code' | 'cortex_code_assistant'
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `cortex remote-control
