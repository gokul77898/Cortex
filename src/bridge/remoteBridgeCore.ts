 ...
 * use CCR v2 transport via CORTEX_USE_CCR_V2. This file is about removing
...
export async function initEnvLessBridgeCore(
  ...
  try {
    transport = await createV2ReplTransport({
      ...
      // process.env.CORTEX_SESSION_ACCESS_TOKEN, which mcp/client.ts
...
async function archiveSession(
  ...
  // cortex-beta + x-organization-uuid are required — without them the
