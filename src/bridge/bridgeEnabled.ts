 ...
 *
 * Remote Control requires a anthropic.com subscription (the bridge auths to CCR
 * with the anthropic.com OAuth token). isCORTEXAISubscriber() excludes
 ...
 * See github.com/deshaw/cortex-issues/issues/24.
 *
 ...
 * (setup-token, CORTEX_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isCORTEXAISubscriber()) {
      return 'Remote Control requires a anthropic.com subscription. Run `cortex auth login` to sign in with your anthropic.com account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `cortex setup-token` or CORTEX_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `cortex auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `cortex auth login` to refresh your account information.'
    }
...
// program, before enableConfigs() runs. isCORTEXAISubscriber() → getGlobalConfig()
...
function isCORTEXAISubscriber(): boolean {
  try {
    return authModule.isCORTEXAISubscriber()
  } catch {
    return false
  }
}
 ...
 * anthropic.com frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of CORTEX (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`cortex update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.CORTEX_CCR_MIRROR) ||
