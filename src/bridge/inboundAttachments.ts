 ...
 * (oauth-authed, same store), write to ~/.cortex/uploads/{sessionId}/, and
 * return @path refs to prepend. CORTEX's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches CORTEX, just without @path.
 */
...
function uploadsDir(): string {
  return join(getCORTEXConfigHomeDir(), 'uploads', getSessionId())
}
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // CORTEX_CUSTOM_OAUTH_URL — keep it inside the try so a bad
