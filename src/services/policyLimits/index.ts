 ...
 * - OAuth users (CORTEX.ai): Only Team and Enterprise/C4E subscribers are eligible
...
function getCachePath(): string {
  return join(getCORTEXConfigHomeDir(), CACHE_FILENAME)
}
...
function getPolicyLimitsEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/cortex_code/policy_limits`
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  if (!isFirstPartyCORTEXBaseUrl()) {
    return false
  }

  // Console users (API key) are eligible if we can get the actual key
  try {
    const { key: apiKey } = getCORTEXApiKeyWithSource({
  ...
  // For OAuth users, check if they have CORTEX.ai tokens
  const tokens = getCORTEXAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have CORTEX.ai inference scope
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for CORTEX.ai users)
