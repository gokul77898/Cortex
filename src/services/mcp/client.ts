...
import type {
  ...
  ContentBlockParam,
  MessageParam,
} from 'src/utils/anthropicMock.js'
...
import { markCORTEXAiMcpConnected } from './cortexai.js'
...
import { isCORTEXInChromeMCPServer } from '../../utils/cortexInChrome/common.js'

// Lazy: toolRendering.tsx pulls React/ink; only needed when CORTEX-in-Chrome MCP server is connected
/* eslint-disable @typescript-eslint/no-require-imports */
const cortexInChromeToolRendering =
  (): typeof import('../../utils/cortexInChrome/toolRendering.js') =>
    require('../../utils/cortexInChrome/toolRendering.js')
...
function getMcpAuthCachePath(): string {
  return join(getCORTEXConfigHomeDir(), 'mcp-needs-auth-cache.json')
}
...
function handleRemoteAuthFailure(
  ...
  transportType: 'sse' | 'http' | 'cortexai-proxy',
): MCPServerConnection {
  ...
  const label: Record<typeof transportType, string> = {
    sse: 'SSE',
    http: 'HTTP',
    'cortexai-proxy': 'anthropic.com proxy',
  }
  logMCPDebug(
    name,
...
/**
 * Fetch wrapper for anthropic.com proxy connections. Attaches the OAuth bearer
 * token and retries once on 401 via handleOAuth401Error (force-refresh).
 *
 * The CORTEX API path has this retry (withRetry.ts, grove.ts) to handle
 * memoize-cache staleness and clock drift. Without the same here, a single
 * stale token mass-401s every anthropic.com connector and sticks them all in the
 * 15-min needs-auth cache.
 */
export function createCORTEXAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      await checkAndRefreshOAuthTokenIfNeeded()
      const currentTokens = getCORTEXAIOAuthTokens()
      if (!currentTokens) {
        throw new Error('No anthropic.com OAuth token available')
      }
    ...
    if (!tokenChanged) {
      ...
      const now = getCORTEXAIOAuthTokens()?.accessToken
 ...
 * See https://github.com/anthropics/cortex-agent-sdk-typescript/issues/202.
  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        const authProvider = new CORTEXAuthProvider(name, serverRef)
        ...
        const wsHeaders = {
          'User-Agent': getMCPUserAgent(),
          ...(serverRef.authToken && {
            'X-CORTEX-Code-Ide-Authorization': serverRef.authToken,
          }),
        }
      ...
      } else if (serverRef.type === 'cortexai-proxy') {
        logMCPDebug(
          name,
          `Initializing anthropic.com proxy transport for server ${serverRef.id}`,
        )

        const tokens = getCORTEXAIOAuthTokens()
        if (!tokens) {
          throw new Error('No anthropic.com OAuth token found')
        }

        const oauthConfig = getOauthConfig()
        const proxyUrl = `${oauthConfig.MCP_PROXY_URL}${oauthConfig.MCP_PROXY_PATH.replace('{server_id}', serverRef.id)}`

        logMCPDebug(name, `Using anthropic.com proxy at ${proxyUrl}`)

        // eslint-disable-next-line eslint-plugin-n/no-unsupported-features/node-builtins
        const fetchWithAuth = createCORTEXAiProxyFetch(globalThis.fetch)
        ...
        logMCPDebug(name, `anthropic.com proxy transport created successfully`)
      } else if (
        (serverRef.type === 'stdio' || !serverRef.type) &&
        isCORTEXInChromeMCPServer(name)
      ) {
        // Run the Chrome MCP server in-process to avoid spawning a ~325 MB subprocess
        const { createChromeContext } = await import(
          '../../utils/cortexInChrome/mcpServer.js'
        )
        const { createCORTEXForChromeMcpServer } = await import(
          '@ant/cortex-for-chrome-mcp'
        )
        const { createLinkedTransportPair } = await import(
          './InProcessTransport.js'
        )
        const context = createChromeContext(serverRef.env)
        inProcessServer = createCORTEXForChromeMcpServer(context)
        ...
        const finalCommand =
          process.env.CORTEX_SHELL_PREFIX || serverRef.command
        const finalArgs = process.env.CORTEX_SHELL_PREFIX
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          serverRef.type === 'cortexai-proxy' &&
          error instanceof Error
        ) {
          logMCPDebug(
            name,
            `anthropic.com proxy connection failed after ${elapsed}ms: ${error.message}`,
          )
          logMCPError(name, error)
          ...
          if (errorCode === 401) {
            return handleRemoteAuthFailure(name, serverRef, 'cortexai-proxy')
          }
        } else if (
      ...
      client.onerror = (error: Error) => {
        ...
        if (
          (transportType === 'http' || transportType === 'cortexai-proxy') &&
      ...
      return toolsToProcess
        .map((tool): Tool => {
          ...
          return {
            ...
            searchHint:
              typeof tool._meta?.['cortex/searchHint'] === 'string'
                ? tool._meta['cortex/searchHint']
                  .replace(/\s+/g, ' ')
                  .trim() || undefined
                : undefined,
            alwaysLoad: tool._meta?.['cortex/alwaysLoad'] === true,
            async description() {
            ...
            async call(
              ...
              const meta = toolUseId
                ? { 'cortexcode/toolUseId': toolUseId }
                : {}
            ...
            ...(isCORTEXInChromeMCPServer(client.name) &&
              (client.config.type === 'stdio' || !client.config.type)
              ? cortexInChromeToolRendering().getCORTEXInChromeMCPToolOverrides(
                tool.name,
              )
              : {}),
            ...(feature('CHICAGO_MCP') &&
...
export async function reconnectMcpServerImpl(
  ...
  try {
    ...
    if (config.type === 'cortexai-proxy') {
      markCORTEXAiMcpConnected(name)
    }
  ...
  try {
    ...
    if (e instanceof Error) {
      ...
      const isConnectionClosedOnHttp =
        ...
        (config.type === 'http' || config.type === 'cortexai-proxy')
