 ...
 * Used by CORTEXAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like CORTEX) should authenticate by including
 ...
 * `cortex mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'cortex mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
  ...
  try {
    ...
    // (instead of CORTEXAuthProvider.saveTokens) to avoid instantiating the
...
export async function performMCPOAuthFlow(
  ...
  // rest of CC's transport wiring (CORTEXAuthProvider.tokens() in client.ts)
  // works unchanged.
  //
  ...
  //
  // Servers with `oauth.xaa` but CORTEX_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set CORTEX_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
  ...
  try {
    ...
    const provider = new CORTEXAuthProvider(
      serverName,
      serverConfig,
      redirectUri,
      true,
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to CORTEX.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/anthropics/cortex-code/issues/28258.
 */
export function wrapFetchWithStepUpDetection(
  baseFetch: FetchLike,
  provider: CORTEXAuthProvider,
): FetchLike {
...
export class CORTEXAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `CORTEX (${this.serverName})`,
  ...
  async refreshAuthorization(
    ...
    const lockfilePath = join(cortexDir, `mcp-refresh-${sanitizedKey}.lock`)
