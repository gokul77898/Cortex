 ...
 * reply and emits notifications/cortex/channel/permission with
 ...
 * capabilities.experimental['cortex/channel/permission'].
 *
 * Kenneth's "would this let CORTEX self-approve?": the approving party is
 * the human via the channel, not CORTEX. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/cortex/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/cortex/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['cortex/channel'] !== undefined &&
      c.capabilities?.experimental?.['cortex/channel/permission'] !== undefined,
  )
}
