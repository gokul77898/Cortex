...
import { fetchCORTEXAIMcpConfigsIfEligible } from './cortexai.js'
...
/**
 * CCR proxy URL path markers. In remote sessions, anthropic.com connectors arrive
...
/**
 * Filter anthropic.com connectors, dropping any whose signature matches an enabled
 ...
 *
 * Connector keys are `anthropic.com <DisplayName>` so they never key-collide with
 ...
 * `mcp__cortex_ai_Slack__*` both hitting mcp.slack.com, ~600 chars/turn wasted).
 *
 ...
 */
export function dedupCORTEXAiMcpServers(
  cortexAiServers: Record<string, ScopedMcpServerConfig>,
  manualServers: Record<string, ScopedMcpServerConfig>,
): {
  servers: Record<string, ScopedMcpServerConfig>
  suppressed: Array<{ name: string; duplicateOf: string }>
  ...
  const servers: Record<string, ScopedMcpServerConfig> = {}
  const suppressed: Array<{ name: string; duplicateOf: string }> = []
  for (const [name, config] of Object.entries(cortexAiServers)) {
    const sig = getMcpServerSignature(config)
    const manualDup = sig !== null ? manualSigs.get(sig) : undefined
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing anthropic.com connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
...
export async function addMcpConfig(
  ...
  if (isCORTEXInChromeMCPServer(name)) {
  ...
  switch (scope) {
    ...
    case 'cortexai':
      throw new Error('Cannot add MCP server to scope: cortexai')
  }
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getCORTEXCodeMcpConfigs().
...
/**
 * Get CORTEX MCP configurations (excludes anthropic.com servers from the
 ...
 * anthropic.com connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns CORTEX server configurations with appropriate scopes
 */
export async function getCORTEXCodeMcpConfigs(
...
/**
 * Get all MCP configurations across all scopes, including anthropic.com servers.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load anthropic.com servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getCORTEXCodeMcpConfigs()
  }

  // Kick off the anthropic.com fetch before getCORTEXCodeMcpConfigs so it overlaps
  // with loadAllPluginsCacheOnly() inside. Memoized — the awaited call below is a cache hit.
  const cortexaiPromise = fetchCORTEXAIMcpConfigsIfEligible()
  const { servers: cortexCodeServers, errors } = await getCORTEXCodeMcpConfigs(
    {},
    cortexaiPromise,
  )
  const { allowed: cortexaiMcpServers } = filterMcpServersByPolicy(
  ...
  // Merge with anthropic.com having lowest precedence
  const servers = Object.assign({}, dedupedCORTEXAi, cortexCodeServers)
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.anthropic.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // https://cortex.slack.com/archives/C093UA0KLD7/p1764975463670109
  return Object.values(configs).every(
    c => c.type === 'sdk' && c.name === 'cortex-vscode',
  )
}
