...
import { getGlobalCORTEXFile } from '../../utils/env.js'
...
export function describeMcpConfigFilePath(scope: ConfigScope): string {
  switch (scope) {
    case 'user':
      return getGlobalCORTEXFile()
    case 'project':
      return join(getCwd(), '.mcp.json')
    case 'local':
      return `${getGlobalCORTEXFile()} [project: ${getCwd()}]`
    case 'dynamic':
...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .cortex/settings.json).
  ...
  // In non-interactive mode (SDK, cortex -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: anthropic.com servers have normalized names starting with "cortex_ai_"
  // but aren't in getMcpConfigByName (they're fetched async separately)
  if (!serverConfig && mcpInfo.serverName.startsWith('cortex_ai_')) {
    return 'cortexai'
  }
