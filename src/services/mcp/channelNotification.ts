 ...
 *   - sends `notifications/cortex/channel` notifications for inbound — this file
 *
 ...
 * Requires anthropic.com OAuth auth — API key users are blocked until
...
export const ChannelMessageNotificationSchema = lazySchema(() =>
  z.object({
    method: z.literal('notifications/cortex/channel'),
    params: z.object({
      content: z.string(),
 ...
 * this declare `capabilities.experimental['cortex/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/cortex/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_METHOD =
  'notifications/cortex/channel/permission'
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/cortex/channel/permission_request'
 ...
 * anthropic.com auth; console orgs have no admin opt-in surface yet.
 *
 *   skip      Not a channel server, or managed org hasn't opted in, or
 *             not in session --channels. Connection stays up; handler
 *             not registered.
 *   register  Subscribe to notifications/cortex/channel.
 *
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['cortex/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/cortex/channel).
  if (!capabilities?.experimental?.['cortex/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare cortex/channel capability',
    }
  }
  ...
  if (!getCORTEXAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires anthropic.com authentication (run /login)',
    }
  }
  ...
  if (entry.kind === 'plugin') {
    // Marketplace verification: the tag is intent (plugin:slack@cortex),
    // the runtime name is just plugin:slack:X — could be slack@cortex or
