  ...
  try {
    if (promptCacheSharingEnabled) {
      try {
        ...
        // budget_tokens via Math.min(budget, maxOutputTokens-1) in cortex.ts,
...
function shouldExcludeFromPostCompactRestore(
  ...
  // Exclude all types of cortex.md files
  // TODO: Refactor to use isMemoryFilePath() from cortexmd.ts for consistency
  // and to also match child directory memory files (.cortex/rules/*.md, etc.)
  try {
