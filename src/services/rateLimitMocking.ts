...
export function checkMockRateLimitError(
  ...
  const status = mockHeaders['cortex-ratelimit-unified-status']
  const overageStatus =
    mockHeaders['cortex-ratelimit-unified-overage-status']
  const rateLimitType =
    mockHeaders['cortex-ratelimit-unified-representative-claim']
