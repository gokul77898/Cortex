// CORTEX voice_stream speech-to-text client for push-to-talk.
//
// Only reachable in ant builds (gated by feature('VOICE_MODE') in useVoice.ts import).
//
// Connects to CORTEX's voice_stream WebSocket endpoint using the same
// OAuth credentials as CORTEX.  The endpoint uses conversation_engine
...
// after CloseStream — the silent-drop signature (anthropics/cortex#287008).
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as CORTEX — available when the
  // user is authenticated with CORTEX (CORTEX.ai subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // visibility.external: true). We target that host instead of anthropic.com
  // because the anthropic.com CF zone uses TLS fingerprinting and challenges
  // non-browser clients (anthropics/cortex-code#34094). Same private-api
  // pod, same OAuth Bearer auth — just a CF zone that doesn't block us.
  // Desktop dictation still uses anthropic.com (Swift URLSession has a
  ...
  // side is anthropics/cortex#278327 + #281372; this lets us ramp
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/cortex-code#40510).
