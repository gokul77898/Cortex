 ...
 *
 * API contract (cortex/cortex#250711 + #283027):
 *   GET  /api/cortex_code/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/cortex_code/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/cortex_code/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
import { getCORTEXCodeUserAgent } from '../../utils/userAgent.js'
...
// Per-entry size cap — server default from cortex/cortex#293258.
...
// GB-tunable per-org (cortex_code_team_memory_limits), so any compile-time
...
export type SyncState = {
   ...
   * (cortex/cortex#293258 adds error_code + extra_details.max_entries).
 ...
 * Format matches the server's entryChecksums values (cortex/cortex#283027)
...
function isUsingOAuth(): boolean {
  if (getAPIProvider() !== 'firstParty' || !isFirstPartyCORTEXBaseUrl()) {
    return false
  }
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/cortex_code/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
  ...
  if (oauthTokens?.accessToken) {
    return {
      headers: {
        ...
        'cortex-beta': OAUTH_BETA_HEADER,
        'User-Agent': getCORTEXCodeUserAgent(),
      },
    }
  }
 ...
 * Requires cortex/cortex#283027 deployed; on failure the caller fails the
...
async function fetchTeamMemoryHashes(
  ...
  try {
    ...
    // Requires cortex/cortex#283027. If entryChecksums is missing,
...
async function uploadTeamMemory(
  ...
  try {
    ...
    // Parse structured 413 (cortex/cortex#293258). The server's
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // 413's extra_details.max_entries — cortex/cortex#293258).  The
  // server's entry-count cap is GB-tunable per-org via
  // cortex_code_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  // Requires cortex/cortex#283027 — if the response lacks entryChecksums
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/cortexmd.js')
...
export async function pushTeamMemory(
  ...
  for (
    ...
    if (!result.conflict) {
      ...
      // max_entries (cortex/cortex#293258), cache it so the next push
    ...
    if (!probe.success || !probe.entryChecksums) {
      // Requires cortex/cortex#283027. A transient probe failure here is
