 ...
 * Based on the backend API contract from cortex/cortex#250711.
 */
...
export const TeamMemoryContentSchema = lazySchema(() =>
  z.object({
    ...
    // cortex/cortex#283027. Optional for forward-compat with older
...
/**
 * Full response from GET /api/cortex_code/team_memory
 */
...
/**
 * Structured 413 error body from the server (cortex/cortex#293258).
...
export type TeamMemorySyncUploadResult = {
  ...
  httpStatus?: number
  /**
   * Structured error_code from a parsed 413 body (cortex/cortex#293258).
