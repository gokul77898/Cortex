...
import { getRelevantCategories, getServerNamesForCategories, SERVER_CATEGORIES } from '../mcpFilter.js'
...
class McpManager {
  ...
  getRelevantToolSchemas(query: string): any[] {
    const categories = getRelevantCategories(query)
    if (categories.size === 0) return []

    const matchedServers = getServerNamesForCategories(categories)
    const relevant = this._tools.filter((t) => matchedServers.has(t.serverName))
