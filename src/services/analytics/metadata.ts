...
import { isCORTEXAISubscriber, getSubscriptionType } from '../../utils/auth.js'
...
import type { EnvironmentMetadata } from '../../types/generated/events_mono/cortex_code/v1/cortex_code_internal_event.js'
 ...
 * - anthropic.com-proxied connectors — always official (from anthropic.com's list)
 * - Servers whose URL matches the official MCP registry — directory
 *   connectors added via `cortex mcp add`, not customer-specific config
 *
...
export function isAnalyticsToolDetailsLoggingEnabled(
  ...
  if (mcpServerType === 'cortexai-proxy') {
    return true
  }
...
export type EnvContext = {
  ...
  isCi: boolean
  isClaubbit: boolean
  isCORTEXCodeRemote: boolean
  isLocalAgentMode: boolean
  isConductor: boolean
  remoteEnvironmentType?: string
  coworkerType?: string
  cortexCodeContainerId?: string
  cortexCodeRemoteSessionId?: string
  tags?: string
  isGithubAction: boolean
  isCORTEXCodeAction: boolean
  isCORTEXAiAuth: boolean
  version: string
  versionBase?: string
  buildTime: string
...
export type EventMetadata = {
  ...
  agentId?: string // CORTEX_AGENT_ID (format: agentName@teamName) or subagent UUID
  parentSessionId?: string // CORTEX_PARENT_SESSION_ID (team lead's session)
...
const buildEnvContext = memoize(async (): Promise<EnvContext> => {
  ...
  return {
    ...
    // the truth. CORTEX_HOST_PLATFORM still overrides for container/remote.
    platformRaw: process.env.CORTEX_HOST_PLATFORM || process.platform,
    arch: env.arch,
    ...
    isCORTEXCodeRemote: isEnvTruthy(process.env.CORTEX_REMOTE),
    isLocalAgentMode: process.env.CORTEX_ENTRYPOINT === 'local-agent',
    isConductor: env.isConductor(),
    ...(process.env.CORTEX_REMOTE_ENVIRONMENT_TYPE && {
      remoteEnvironmentType: process.env.CORTEX_REMOTE_ENVIRONMENT_TYPE,
    }),
    // Gated by feature flag to prevent leaking "coworkerType" string in external builds
    ...(feature('COWORKER_TYPE_TELEMETRY')
      ? process.env.CORTEX_COWORKER_TYPE
        ? { coworkerType: process.env.CORTEX_COWORKER_TYPE }
        : {}
      : {}),
    ...(process.env.CORTEX_CONTAINER_ID && {
      cortexCodeContainerId: process.env.CORTEX_CONTAINER_ID,
    }),
    ...(process.env.CORTEX_REMOTE_SESSION_ID && {
      cortexCodeRemoteSessionId: process.env.CORTEX_REMOTE_SESSION_ID,
    }),
    ...(process.env.CORTEX_TAGS && {
      tags: process.env.CORTEX_TAGS,
    }),
    isGithubAction: isEnvTruthy(process.env.GITHUB_ACTIONS),
    isCORTEXCodeAction: isEnvTruthy(process.env.CORTEX_ACTION),
    isCORTEXAiAuth: isCORTEXAISubscriber(),
    ...
    ...(isEnvTruthy(process.env.GITHUB_ACTIONS) && {
      ...
      githubActionRef: process.env.GITHUB_ACTION_PATH?.includes(
        ...
        ? process.env.GITHUB_ACTION_PATH.split('cortex-code-action/')[1]
        : undefined,
    }),
...
export async function getEventMetadata(
  ...
  const metadata: EventMetadata = {
    ...
    envContext,
    ...(process.env.CORTEX_ENTRYPOINT && {
      entrypoint: process.env.CORTEX_ENTRYPOINT,
    }),
...
export type FirstPartyEventLoggingMetadata = {
  ...
  process?: string
  // auth is a top-level field on CORTEXCodeInternalEvent (proto PublicApiAuth).
  // account_id is intentionally omitted — only UUID fields are populated client-side.
  auth?: PublicApiAuth
  // core fields correspond to the top level of CORTEXCodeInternalEvent.
  ...
  // CORTEXCodeInternalEvent proto. Includes but is not limited to information
...
export function to1PEventFormat(
  ...
  //   event_schemas/.../cortex_code/v1/cortex_code_internal_event.proto
  ...
  const env: EnvironmentMetadata = {
    ...
    is_cortex_code_remote: envContext.isCORTEXCodeRemote,
    ...
    is_cortex_code_action: envContext.isCORTEXCodeAction,
    is_cortex_ai_auth: envContext.isCORTEXAiAuth,
  ...
  if (envContext.cortexCodeContainerId) {
    env.cortex_code_container_id = envContext.cortexCodeContainerId
  }
  if (envContext.cortexCodeRemoteSessionId) {
    env.cortex_code_remote_session_id = envContext.cortexCodeRemoteSessionId
  }
  if (envContext.tags) {
  ...
  // parts of CORTEXCodeInternalEvent deduplicated.
  ...
  // rather than at the top level of CORTEXCodeInternalEvent
