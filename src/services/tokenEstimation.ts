...
import type { BetaMessageParam as MessageParam } from 'src/utils/anthropicMock.js'
...
import { getAPIMetadata, getExtraBodyParams } from './api/cortex.js'
...
function hasThinkingBlocks(
  messages: CORTEX.Beta.Messages.BetaMessageParam[],
): boolean {
...
function stripToolSearchFieldsFromMessages(
  messages: CORTEX.Beta.Messages.BetaMessageParam[],
): CORTEX.Beta.Messages.BetaMessageParam[] {
  return messages.map(message => {
    ...
    const normalizedContent = message.content.map(block => {
      ...
      if (block.type === 'tool_use') {
        ...
        const toolUse =
          block as CORTEX.Beta.Messages.BetaToolUseBlockParam & {
            caller?: unknown
          }
        return {
      ...
      if (block.type === 'tool_result') {
        const toolResult =
          block as CORTEX.Beta.Messages.BetaToolResultBlockParam
...
export async function countTokensWithAPI(
  ...
  const message: CORTEX.Beta.Messages.BetaMessageParam = {
    role: 'user',
    content: content,
  }
...
export async function countMessagesTokensWithAPI(
  messages: CORTEX.Beta.Messages.BetaMessageParam[],
  tools: CORTEX.Beta.Messages.BetaToolUnion[],
): Promise<number | null> {
  return withTokenCountVCR(messages, tools, async () => {
    try {
      ...
      if (getAPIProvider() === 'bedrock') {
        // @cortex-sdk/bedrock-sdk doesn't support countTokens currently
      ...
      const response = await cortex.beta.messages.countTokens({
        model: normalizeModelStringForAPI(model),
        messages:
...
export async function countTokensViaHaikuFallback(
  ...
  const isBedrockWithThinking =
    isEnvTruthy(process.env.CORTEX_USE_BEDROCK) && containsThinking
  // If we're on Vertex with thinking blocks, use Sonnet since Haiku 3.5 doesn't support thinking
  const isVertexWithThinking =
    isEnvTruthy(process.env.CORTEX_USE_VERTEX) && containsThinking
  ...
  const response = await cortex.beta.messages.create({
    model: normalizeModelStringForAPI(model),
  ...
  if (
    ...
    return roughTokenCountEstimationForContent(
      message.message?.content as
        | string
        | Array<CORTEX.ContentBlock>
        | Array<CORTEX.ContentBlockParam>
        | undefined,
    )
  }
...
function roughTokenCountEstimationForBlock(
  block: string | CORTEX.ContentBlock | CORTEX.ContentBlockParam,
): number {
  ...
  if (block.type === 'image' || block.type === 'document') {
    // https://platform.anthropic.com/docs/en/build-with-cortex/vision#calculate-image-costs
...
async function countTokensWithBedrock({
  ...
  try {
    ...
    const requestBody = {
      cortex_version: 'bedrock-2023-05-31',
      // When we pass tools and no messages, we need to pass a dummy message
      // to get an accurate tool token count.
      messages:
        messages.length > 0 ? messages : [{ role: 'user', content: 'foo' }],
      ...
      ...(betas.length > 0 && { cortex_beta: betas }),
      ...(containsThinking && {
        thinking: {
          type: 'enabled',
          budget_tokens: TOKEN_COUNT_THINKING_BUDGET,
