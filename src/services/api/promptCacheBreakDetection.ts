...
import { getCORTEXTempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getCORTEXTempDir(), `cache-break-${suffix}.diff`)
}
...
type PreviousState = {
   ...
   *  (sticky-on latched in cortex.ts). Tracked to verify the fix. */
   ...
   *  or cortex_internal.effort_override. */
  effortValue: string
  /** Hash of getExtraBodyParams() — catches CORTEX_EXTRA_BODY and
   *  cortex_internal changes. */
...
// CORTEX's server-side prompt cache TTL thresholds to test.
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getCORTEXTempDir(), { recursive: true })
