...
describe('getRateLimitResetDelayMs - CORTEX (firstParty)', () => {
  test('reads cortex-ratelimit-unified-reset Unix timestamp', async () => {
    ...
    const error = makeError({
      'cortex-ratelimit-unified-reset': String(futureUnixSec),
    })
  ...
  test('returns null when reset is in the past', async () => {
    ...
    const error = makeError({
      'cortex-ratelimit-unified-reset': String(pastUnixSec),
    })
    expect(getRateLimitResetDelayMs(error)).toBeNull()
  })
})
...
describe('getRateLimitResetDelayMs - providers without reset headers', () => {
  test('returns null for bedrock', async () => {
    process.env.CORTEX_USE_BEDROCK = '1'
    const { getRateLimitResetDelayMs } =
      await importFreshWithRetryModule('bedrock')
    const error = makeError({ 'cortex-ratelimit-unified-reset': String(Math.floor(Date.now() / 1000) + 60) })
    // Bedrock doesn't use this header — should still return null
    expect(getRateLimitResetDelayMs(error)).toBeNull()
  })

  test('returns null for vertex', async () => {
    process.env.CORTEX_USE_VERTEX = '1'
    const { getRateLimitResetDelayMs } =
