 ...
 * The CORTEX SDK wraps underlying errors in the `cause` property.
 ...
 * - Standard CORTEX API: `{ error: { error: { message: "..." } } }`
 ...
 * 1. `error.error.error.message` — standard CORTEX API shape
...
function extractNestedErrorMessage(error: APIError): string | null {
  ...
  // Standard CORTEX API shape: { error: { error: { message } } }
