...
const originalUseGithub = process.env.CORTEX_USE_GITHUB

afterEach(() => {
  if (originalUseGithub === undefined) {
    delete process.env.CORTEX_USE_GITHUB
  } else {
    process.env.CORTEX_USE_GITHUB = originalUseGithub
  }
})
...
test('resolveProviderRequest applies GitHub normalization when CORTEX_USE_GITHUB=1', () => {
