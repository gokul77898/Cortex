...
import { hasProfileScope, isCORTEXAISubscriber } from '../../utils/auth.js'
...
// N `cortex -p` invocations into ~1 API call/day.
...
async function _fetchMetricsEnabled(): Promise<MetricsEnabledResponse> {
  ...
  const endpoint = `https://api.anthropic.com/api/cortex_code/organizations/metrics_enabled`
...
export async function checkMetricsEnabled(): Promise<MetricsStatus> {
  ...
  if (isCORTEXAISubscriber() && !hasProfileScope()) {
    return { enabled: false, hasError: false }
  }
