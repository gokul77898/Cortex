...
import {
  codexStreamToCORTEX,
  convertCORTEXMessagesToResponsesInput,
...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'cortex-codex-'))
  tempDirs.push(dir)
...
async function collectStreamEventTypes(responseText: string): Promise<string[]> {
  ...
  const events: string[] = []
  for await (const event of codexStreamToCORTEX(new Response(stream), 'gpt-5.4')) {
...
describe('Codex request translation', () => {
  ...
  test('converts assistant tool use and user tool result into Responses items', () => {
    const items = convertCORTEXMessagesToResponsesInput([
      {
        role: 'assistant',
        content: [
  ...
  test('converts completed Codex tool response into CORTEX message', () => {
    const message = convertCodexResponseToCORTEXMessage(
      {
        id: 'resp_1',
  ...
  test('translates Codex SSE text stream into CORTEX events', async () => {
    const responseText = [
