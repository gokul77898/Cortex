 ...
 */
export function shouldUseCORTEXAIAuth(scopes: string[] | undefined): boolean {
...
export function buildAuthUrl({
  ...
  port: number
  isManual: boolean
  loginWithCORTEXAi?: boolean
  inferenceOnly?: boolean
  orgUUID?: string
  loginHint?: string
  loginMethod?: string
}): string {
  const authUrlBase = loginWithCORTEXAi
    ? getOauthConfig().CLAUDE_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show CORTEX Max upsell
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full CORTEX AI set. The
  ...
  try {
    ...
    // CORTEX_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
    ...
    const existing = getCORTEXAIOAuthTokens()
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.CORTEX_ACCOUNT_UUID
  const envUserEmail = process.env.CORTEX_USER_EMAIL
  const envOrganizationUuid = process.env.CORTEX_ORGANIZATION_UUID
