 ...
 *   CORTEX_OAUTH_TOKEN / FD, or keychain tokens missing metadata): Eligible —
...
export function isRemoteManagedSettingsEligible(): boolean {
  ...
  // Check OAuth first: most CORTEX.ai users have no API key in the keychain.
  ...
  // Externally-injected tokens (CCD via CORTEX_OAUTH_TOKEN, CCR via
  // CORTEX_OAUTH_TOKEN_FILE_DESCRIPTOR, Agent SDK, CI) carry no
  // subscriptionType metadata — getCORTEXAIOAuthTokens() constructs them with
  ...
  // Wrap in try-catch because getCORTEXApiKeyWithSource throws in CI/test environments
