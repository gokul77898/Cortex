...
import { CORTEXError } from '../utils/errors.js'
import { normalizePathForComparison, pathsEqual } from '../utils/file.js'
import { getConnectedIdeClient } from '../utils/ide.js'
import { jsonParse } from '../utils/slowOperations.js'

class DiagnosticsTrackingError extends CORTEXError {}
...
export class DiagnosticTrackingService {
  ...
  // Map<normalizedPath, lastCORTEXFsRightDiagnostics>
  ...
  /**
   * Get new diagnostics from file://, _cortex_fs_right, and _cortex_fs_ URIs that aren't in the baseline.
  ...
  async getNewDiagnostics(): Promise<DiagnosticFile[]> {
    ...
    const diagnosticsForCORTEXFsRightUrisWithBaselinesMap = new Map<
      string,
      DiagnosticFile
    >()
    allDiagnosticFiles
      .filter(file => this.baseline.has(this.normalizeFileUri(file.uri)))
      .filter(file => file.uri.startsWith('_cortex_fs_right:'))
      .forEach(file => {
        diagnosticsForCORTEXFsRightUrisWithBaselinesMap.set(
    ...
    for (const file of diagnosticsForFileUrisWithBaselines) {
      ...
      // Get the _cortex_fs_right file if it exists
      const cortexFsRightFile =
        diagnosticsForCORTEXFsRightUrisWithBaselinesMap.get(normalizedPath)

      // Determine which file to use based on the state of right file diagnostics
      let fileToUse = file

      if (cortexFsRightFile) {
        const previousRightDiagnostics =
          this.rightFileDiagnosticsState.get(normalizedPath)

        // Use _cortex_fs_right if:
        ...
        if (
          ...
          !this.areDiagnosticArraysEqual(
            previousRightDiagnostics,
            cortexFsRightFile.diagnostics,
          )
        ) {
          fileToUse = cortexFsRightFile
        }
