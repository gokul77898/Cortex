/**
 * Main entrypoint for CORTEX Agent SDK types.
 *
 ...
 *
 * If your SDK MCP calls will run longer than 60s, override CORTEX_STREAM_CLOSE_TIMEOUT
 */
...
/**
 * A scheduled task from `<dir>/.cortex/scheduled_tasks.json`.
 * @internal
 */
...
/**
 * Watch `<dir>/.cortex/scheduled_tasks.json` and yield events as tasks fire.
 *
...
/**
 * A user message typed on anthropic.com, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a anthropic.com remote-control bridge connection from a daemon process.
 *
 ...
 * anthropic.com keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on anthropic.com) into `query()`'s input
