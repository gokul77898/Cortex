...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('CORTEX decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls CORTEX's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
...
export const McpCORTEXAIProxyServerConfigSchema = lazySchema(() =>
  z.object({
    type: z.literal('cortexai-proxy'),
    url: z.string(),
    id: z.string(),
  }),
)

// Broader config type for status responses (includes cortexai-proxy which is output-only)
export const McpServerStatusConfigSchema = lazySchema(() =>
  z.union([
    McpServerConfigForProcessTransportSchema(),
    McpCORTEXAIProxyServerConfigSchema(),
  ]),
)
  ...
  z
    .object({
      ...
      scope: z
        ...
        .optional()
        .describe(
          'Configuration scope (e.g., project, user, local, cortexai, managed)',
        ),
      tools: z
        .array(
          z.object({
      ...
      capabilities: z
        ...
        .optional()
        .describe(
          "@internal Server capabilities (available when connected). experimental['cortex/channel'] is only present if the server's plugin is on the approved channels allowlist — use its presence to decide whether to show an Enable-channel prompt.",
        ),
    })
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (CORTEX decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
  ...
  z
    .object({
      ...
      apiProvider: z
        ...
        .optional()
        .describe(
          'Active API backend. CORTEX OAuth login only applies when "firstParty"; for 3P providers the other fields are absent and auth is external (AWS creds, gcloud ADC, etc.).',
        ),
    })
  ...
  z
    .object({
      ...
      model: z
        ...
        .optional()
        .describe(
          "Model alias (e.g. 'sonnet', 'opus', 'haiku') or full model ID (e.g. 'cortex-opus-4-5'). If omitted or 'inherit', uses the main model",
        ),
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.cortex/agent-memory/<agentType>/, 'project' - .cortex/agent-memory/<agentType>/, 'local' - .cortex/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.cortex/settings.json). " +
        "'project' - Project settings (.cortex/settings.json). " +
        "'local' - Local settings (.cortex/settings.local.json).",
    ),
)
...
/** Placeholder for APIUserMessage from src/utils/anthropicMock.js */
export const APIUserMessagePlaceholder = lazySchema(() => z.unknown())

/** Placeholder for APIAssistantMessage from src/utils/anthropicMock.js */
export const APIAssistantMessagePlaceholder = lazySchema(() => z.unknown())

/** Placeholder for RawMessageStreamEvent from src/utils/anthropicMock.js */
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for anthropic.com subscription users.'),
)
