 ...
 * CORTEX OAuth token. Backed by the memoized getCORTEXAIOAuthTokens —
...
export function hasVoiceAuth(): boolean {
  // Voice mode requires CORTEX OAuth — it uses the voice_stream
  // endpoint on anthropic.com which is not available with API keys,
  // Bedrock, Vertex, or Foundry.
  if (!isCORTEXAuthEnabled()) {
    return false
  }
  // isCORTEXAuthEnabled only checks the auth *provider*, not whether
