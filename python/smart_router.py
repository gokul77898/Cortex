...
smart_router.py
---------------
Intelligent auto-router for cortex.
...
Contribution to: https://github.com/Gitlawb/cortex
"""
...
def build_default_providers() -> list[Provider]:
    ...
    return [
        ...
        Provider(
            name="huggingface",
            ping_url=f"{os.getenv('HF_BASE_URL', 'https://router.huggingface.co/v1')}/models",
...
class SmartRouter:
    """
    Intelligently routes CORTEX API requests to the best
    ...
    def get_model_for_provider(
        ...
        """Map a CORTEX model name to the provider's actual model."""
        if is_large_request:
            return provider.big_model
        is_large = any(
            keyword in cortex_model.lower()
    ...
    async def route(
        self,
        messages: list[dict],
        cortex_model: str = "cortex-sonnet",
        attempt: int = 0,
