...
def test_get_model_large_request():
    ...
    r = make_router()
    model = r.get_model_for_provider(p, "cortex-sonnet")
    assert model == "openai-big"


def test_get_model_large_message_overrides_cortex_label():
    p = make_provider("openai")
    r = make_router()
    model = r.get_model_for_provider(p, "cortex-haiku", is_large_request=True)
    assert model == "openai-big"
...
def test_get_model_small_request():
    p = make_provider("openai")
    r = make_router()
    model = r.get_model_for_provider(p, "cortex-haiku")
...
async def test_route_returns_best_provider():
    ...
    result = await r.route([{"role": "user", "content": "Hi"}], "cortex-haiku")
