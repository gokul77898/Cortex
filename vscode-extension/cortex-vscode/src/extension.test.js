...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'cortex',
    launchCommand: 'cortex --project-aware',
    terminalName: 'CORTEX',
    shimEnabled: false,
    workspaceFolder: '/workspace/cortex/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/cortex/very/long/path/example-project',
    launchCwdLabel: '/workspace/cortex/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/cortex/very/long/path/example-project/.cortex-profile.json',
    workspaceProfilePath: '/workspace/cortex/very/long/path/example-project/.cortex-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the CORTEX wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(html, /Open<span class="wordmark-accent">CORTEX<\/span>/);
  ...
  assert.match(
    html,
    /title="\/workspace\/cortex\/very\/long\/path\/example-project"[^>]*>\/workspace\/cortex\/very\/long\/path\/example-project<\//,
  );
});
...
test('CORTEXControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { CORTEXControlCenterProvider } = loadExtension();
  const provider = new CORTEXControlCenterProvider();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/cortex/src/panels/control-center.js',
      workspacePath: '/workspace/cortex',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/cortex/src/panels',
      projectAwareCwdLabel: '/workspace/cortex/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/cortex',
      workspaceRootCwdLabel: '/workspace/cortex',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/cortex',
      activeFilePath: '/workspace/cortex/src/panels/control-center.js',
      workspacePath: '/workspace/cortex',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/cortex',
      projectAwareCwdLabel: '/workspace/cortex',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/cortex/src/panels',
      launchCwdLabel: '/workspace/cortex/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/cortex\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/cortex/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/cortex',
      launchCwdLabel: '/workspace/cortex',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/cortex/);
  assert.match(html, /Same workspace-root target as Launch CORTEX because the relative command resolves from the workspace root · \/workspace\/cortex/);
});
