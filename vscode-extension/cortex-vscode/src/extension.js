...
const CORTEX_REPO_URL = 'https://github.com/Gitlawb/cortex';
const CORTEX_SETUP_URL = 'https://github.com/Gitlawb/cortex/blob/main/README.md#quick-start';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('cortex');
  const launchCommand = configured.get('launchCommand', 'cortex');
  const terminalName = configured.get('terminalName', 'CORTEX');
...
async function launchCORTEX(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `CORTEX command not found: ${executable}. Install it with: npm install -g @gitlawb/cortex`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(CORTEX_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(CORTEX_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch CORTEX because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="CORTEX wordmark">Open<span class="wordmark-accent">CORTEX</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream CORTEX project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and CORTEX commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class CORTEXControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchCORTEX({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const startCommand = vscode.commands.registerCommand('cortex.start', async () => {
    await launchCORTEX();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'cortex.startInWorkspaceRoot',
    async () => {
      await launchCORTEX({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('cortex.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(CORTEX_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'cortex.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(CORTEX_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'cortex.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('cortex.openControlCenter', async () => {
